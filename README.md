...
Every governed tool outcome is retained before the 64,000-character model-facing formatter can truncate it. The sensitive local artifact lives under `.agent/logs/tool-responses/<request-id>/`: `manifest.json` records loop/run/request/audit correlation, outcome, exact SHA-256, character/UTF-8 sizes, and ordered chunk metadata, while UTF-16-safe 32,000-character chunks preserve the complete bounded response for later inspection. Responses are bounded at 160,000 characters; retention keeps at most 256 complete artifacts and 64 MiB, evicting only the oldest complete artifact under a cross-process workspace lock. Exact orphaned staging directories are recovered under that lock. The returned model text places the manifest path and integrity metadata before output, so those references survive final truncation; a retention failure is explicitly reported instead of advertising partial evidence. Raw response content never enters audit. The general logs tree remains denied to agent tools, while the more-specific tool-response path requires human approval for read/list inspection.
