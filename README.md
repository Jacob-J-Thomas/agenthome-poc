...
Run discovery uses immutable creation-order opaque cursors and a crash-repairable lightweight metadata index, so continuation pages do not reparse every retained trace. The Web surface fetches the selected loop through the exact loop-id filter alongside workspace discovery and progressively loads that loop's older live traces and tombstones without dropping the current selection. Deleted definitions remain available as archived history as older pages are discovered.
