...
Completed invocation receipts remain replayable for at least 30 days. Cleanup runs only when a receipt write reaches the governed 10,000-receipt or 128 MiB workspace ceiling; it never selects pending receipts or completed receipts newer than the replay cutoff. A cross-process mutation lease and one bounded durable batch journal bind the exact selected operation IDs, completion timestamps, hashes, and byte counts. The intent audit precedes deletion, partial deletion resumes from the journal after owner expiry, and an outcome audit is attempted by one durable owner; an interrupted or failed outcome attempt becomes a durable audit warning instead of being duplicated. Malformed, oversized, unexpectedly named, over-count, or changed receipt artifacts fail closed before a new cleanup batch deletes anything. After an expired receipt is pruned, that receipt alone no longer reserves its operation ID for replay, although surviving run evidence can still reconcile or reject a later request.
