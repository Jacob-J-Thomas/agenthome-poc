...
After reload or reconnect, a selected nonterminal run is monitored through a cached exact-ID index lookup and an opaque ETag bound to both the complete summary and canonical artifact hash. Standard weak, wildcard, and combined `If-None-Match` validators are honored. Unchanged polls return no body, including during long approval waits; verified compact metadata serves those polls without reopening the full trace or rescanning every loop directory, while filesystem change notifications invalidate the affected artifact binding and run-ID ownership caches. A watcher error discards affected certainty and recreates the watcher so complete canonical validation can re-establish lightweight caching. Every changed validator reloads workspace pages, quota, full run state, and trace evidence even when the compact summary is unchanged, because only `304 Not Modified` proves the artifact-bound evidence is current. A changed validator is retained only after that full refresh succeeds. If the monitor endpoint is unavailable, scheduled monitoring falls back to ordinary full evidence with bounded exponential backoff; two exact-ID misses with no successful full-evidence read between them clear stale active evidence.
