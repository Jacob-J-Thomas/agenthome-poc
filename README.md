...
6. `Persistence.Memory.ConversationMemoryStore` starts each runtime session with a fresh active transcript by moving any non-empty `conversations/current.ndjson` into `conversations/archive/` before the default conversation loop accepts prompts.
...
8. `ConversationRuntimeState` starts with seeded runtime context and a fresh transcript. A user can explicitly load an old transcript with `/history`, `/conversations`, or `/load` before the first model turn, or start another fresh transcript in-place with `/new`.
