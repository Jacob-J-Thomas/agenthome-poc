...
- Durable planning commands, custom-loop authoring or invocation, background loop execution, cron scheduling, wake listeners, hooks, skills, subagents, MCP behavior, and `plan_*` governed tool commands are not implemented in the current source. The persisted default conversation definition and its run records are live cutover foundations, not proof that editable custom loops exist. Do not describe `/plan`, `.agent/tasks/plan.json`, custom-loop execution, or automatic trigger delivery as live behavior unless source is updated in the same change.
