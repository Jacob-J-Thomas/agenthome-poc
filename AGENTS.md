...
- The current CLI `run` path loads the nearest `AGENTS.md` found by walking upward from `--workdir`, plus `.agent/ROLE.md`, `.agent/SOUL.md`, `.agent/PERSONALITY.md`, `.agent/CONTEXT.md`, `.agent/MEMORY.md`, and `.agent/models.json` under `--workdir` as startup runtime context when those files exist and are non-empty. `ROLE.md` is the contextual workspace role, while `SOUL.md` and `PERSONALITY.md` are durable agent identity instructions. A legacy `.agent/AGENT.md` is read only when `ROLE.md` is absent.
