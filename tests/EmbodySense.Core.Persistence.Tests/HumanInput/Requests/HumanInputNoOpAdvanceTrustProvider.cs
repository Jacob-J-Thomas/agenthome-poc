...
internal sealed class HumanInputNoOpAdvanceTrustProvider(ICapabilityCatalogTrustProvider inner) : ICapabilityCatalogTrustProvider
{
    ...
    public async Task<CapabilityCatalogTrustState> AdvanceAsync(
        ...
        => await inner.ReadAsync(workspaceIdentity, cancellationToken)
            ?? throw new IOException("The test trust anchor was not initialized.");
}
