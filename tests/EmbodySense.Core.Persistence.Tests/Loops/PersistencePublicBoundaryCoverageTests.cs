...
public sealed class PersistencePublicBoundaryCoverageTests
{
    ...
    public void Artifact_serializer_rejects_cross_event_tool_protocol_mismatches()
    {
        ...
        RejectRun(RemoveEvents(run, 1));
        RejectRun(InsertEvent(run, 2, run.Events[1] with { EventId = "event-reservation-duplicate" }));
        RejectRun(ReplaceEvidence(run, 2, evidence => evidence with { TargetPath = "different-target" }));

        var differentAuthority = ToolAuthority() with { Detail = "Different current authority detail." };
        RejectRun(ReplaceEvidence(run, 2, evidence => evidence with { Authority = differentAuthority }, differentAuthority));
        RejectRun(InsertEvent(run, 3, run.Events[2] with { EventId = "event-governance-duplicate" }));
        RejectRun(RemoveEvents(run, 2));
        RejectRun(RemoveEvents(run, 3));
        RejectRun(ReplaceEvidence(run, 4, evidence => evidence with { Governance = evidence.Governance! with { ApprovalDetail = "Mismatched approval detail." } }));

        var integrity = CreateToolRun(includeIntegrity: true);
        RejectRun(ReplaceEvidence(integrity, 5, evidence => evidence with { CanonicalResultHash = new string('0', 64) }));
    }

    [Fact]
    public void Artifact_deserializer_rejects_compact_tool_protocol_order_shape_and_reference_mismatches()
    {
        RejectTool(root => ToolEvidence(root, 1)["phase"] = "governanceDecided");
        RejectTool(root =>
        {
            var events = Events(root);
            var governance = events[2]!.DeepClone();
            events.RemoveAt(2);
            events.Insert(1, governance);
        });
        RejectTool(root => Events(root).Insert(3, Events(root)[2]!.DeepClone()));
        RejectTool(root => Events(root).RemoveAt(2));
        RejectTool(root => Events(root).RemoveAt(3));
        RejectTool(root => ToolEvidence(root, 4)["outcomeSequence"] = 999);
        RejectTool(root => ToolEvidence(root, 4)["shape"] = 99);
        RejectTool(root => ToolEvidence(root, 2)["extra"] = true);
        RejectTool(root => ToolEvidence(root, 1)["shape"] = "one");
        ...
        RejectTool(root =>
        {
            ...
            Events(root)[2]!["toolAuthority"] = new JsonObject { ["$authority"] = "a1" };
        });
    }
    ...
    {
        RejectTool(root => ToolEvidence(root, 5)["hasGovernance"] = "yes", includeIntegrity: true);
        RejectTool(root =>
        {
            var events = Events(root);
            events.RemoveAt(4);
            events.RemoveAt(3);
            events.RemoveAt(2);
            var marker = ToolEvidence(root, 2);
    ...
    private static CustomLoopRunRecord CreateRun()
    {
        ...
        var run = new CustomLoopRunRecord(CustomLoopRunRecord.CurrentSchemaVersion, "run-boundary", definition.Id, 1, CustomLoopRunStatus.Admitted, Timestamp, Timestamp, null, "web", new CustomLoopModelSnapshot("openai", "gpt-5"), "invoke-boundary", string.Empty, definition, "Initial prompt", null, context, CustomLoopExecutionClock.NotStarted(), CustomLoopRunCheckpoint.Start(), [admitted], null, null, null);
    ...
    private static CustomLoopRunRecord CreateToolRun(bool includeIntegrity = false)
    {
        ...
        var events = new List<CustomLoopRunEvent>
        {
            ...
            ToolEvent(2, "event-reservation", CustomLoopRunEventKind.ToolRequestReserved, reservation, authority),
            ToolEvent(3, "event-governance", CustomLoopRunEventKind.ToolGovernanceDecided, governed, authority),
            ToolEvent(4, "event-outcome", CustomLoopRunEventKind.ToolOutcomeObserved, outcome, authority),
            ToolEvent(5, "event-returned", CustomLoopRunEventKind.ToolOutcomeObserved, returned, authority)
        };
        if (includeIntegrity)
        {
            ...
            events.Add(ToolEvent(6, "event-integrity", CustomLoopRunEventKind.ToolIntegrityFailed, integrity, authority));
        }

        var checkpoint = CustomLoopRunCheckpoint.Start() with { ToolRequestsUsed = 1 };
        var run = new CustomLoopRunRecord(CustomLoopRunRecord.CurrentSchemaVersion, "run-tool-boundary", definition.Id, events.Count, CustomLoopRunStatus.Admitted, Timestamp, Timestamp, null, "web", new CustomLoopModelSnapshot("openai", "gpt-5"), "invoke-tool-boundary", string.Empty, definition, "Initial prompt", null, CustomLoopContextSnapshot.CreateEmpty(Timestamp), CustomLoopExecutionClock.NotStarted(), checkpoint, events.ToArray(), null, null, null);
        return CustomLoopAdmissionRequestHash.Apply(run);
    }
