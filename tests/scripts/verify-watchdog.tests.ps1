...
$broadApplicationVerifierSharedWavePhases += @(
    ...
    [pscustomobject]@{ Name = "format-changed"; EstimatedDurationSeconds = 90; Weight = 3; ResourceClass = "CpuBound" }
...
Assert-Equal -Actual $broadApplicationVerifierSharedWaveSeconds -Expected 440 -Message "The complete broad Application-plus-verifier shared wave must retain its calibrated scheduler critical path."
...
Assert-True -Condition ($qualificationScript.IndexOf('Add-QualificationPhase -Name "format-changed" -FileName "dotnet" -Arguments $formatArguments -TimeoutSeconds 120 -EstimatedDurationSeconds 90 -Weight $qualificationCpuBoundWeight -ResourceClass "CpuBound"', [StringComparison]::Ordinal) -ge 0) -Message "Changed-file formatting must retain its measured Windows-contended estimate, child bound, and CPU posture."
