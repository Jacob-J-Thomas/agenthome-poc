...
Assert-True -Condition ($persistenceScheduleProfile.Weight -eq 2 -and $startupScheduleProfile.Weight -eq 2) -Message "The two Windows-dominant suites must retain two-unit resource weight."
Assert-True -Condition (@($script:QualificationTestScheduleProfiles | Where-Object { $_.EstimatedDurationSeconds -le 20 -and $_.Weight -ne 1 }).Count -eq 0) -Message "Short qualification suites must retain one-unit backfill weight."
Assert-True -Condition ($script:QualificationMaximumProcessHeavyWorkers -eq 4 -and $script:QualificationContractWeight -eq 1) -Message "Qualification must admit at most four one-unit process-heavy workers while dominant suites remain capacity-bounded."
try {
...
Assert-True -Condition ($qualificationScript.IndexOf('-Weight $testScheduleProfile.Weight -ResourceClass "ProcessHeavy"', [StringComparison]::Ordinal) -ge 0) -Message "Every selected test project must use its measured resource weight."
Assert-True -Condition ($qualificationScript.IndexOf('-Weight $script:QualificationContractWeight -ResourceClass "ProcessHeavy"', [StringComparison]::Ordinal) -ge 0) -Message "Verifier contracts must use their checked one-unit backfill weight."
