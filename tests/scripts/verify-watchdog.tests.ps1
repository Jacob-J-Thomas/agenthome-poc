...
$boundedPhasesScheduleProfile = Get-QualificationContractScheduleProfile -ScriptName "verify-bounded-phases.tests.ps1"
Assert-True -Condition ($boundedPhasesScheduleProfile.EstimatedDurationSeconds -eq 45 -and $boundedPhasesScheduleProfile.TimeoutSeconds -eq 90 -and $boundedPhasesScheduleProfile.Weight -eq 1 -and $boundedPhasesScheduleProfile.ResourceClass -ceq "ProcessLight" -and $boundedPhasesScheduleProfile.Isolation -ceq "Shared") -Message "The functional redirected-output timeout proof must retain its honest bounded qualification profile."
...
Assert-True -Condition ($watchdogScheduleProfile.EstimatedDurationSeconds -eq 120 -and $watchdogScheduleProfile.TimeoutSeconds -eq 150 -and $watchdogScheduleProfile.Weight -eq 1 -and $watchdogScheduleProfile.ResourceClass -ceq "ProcessLight" -and $watchdogScheduleProfile.Isolation -ceq "Shared") -Message "The source-heavy watchdog contract must retain measured Windows scan priority and bounded headroom."
...
$broadApplicationVerifierSharedWavePhases += @(
    ...
    [pscustomobject]@{ Name = "format-changed"; EstimatedDurationSeconds = 120; Weight = 3; ResourceClass = "CpuBound" }
...
$expectedSharedDirectTestConsumers = @(
    "tests/EmbodySense.Core.Application.Tests/HumanReview/CurrentHumanReviewContinuationAuthoritySourceTests.cs",
...
Assert-Equal -Actual $linkedApplicationFixturePlan.TestProjects.Count -Expected 3 -Message "A linked Application fixture must select the Application, Persistence, and Startup consumers."
...
Assert-True -Condition ($linkedApplicationFixturePlan.TestProjects -ccontains "tests/EmbodySense.Core.Persistence.Tests/EmbodySense.Core.Persistence.Tests.csproj") -Message "A linked Application fixture must execute its Persistence consumer."
...
Assert-VerificationWatchdogDeadlineContract -Qualification $false -VerificationComponent "nestedprocess" -DeadlineSeconds 600
Assert-VerificationWatchdogDeadlineContract -Qualification $false -VerificationComponent "MacOSPlatformContract" -DeadlineSeconds 600
Assert-VerificationWatchdogDeadlineContract -Qualification $false -VerificationComponent "macosplatformcontract" -DeadlineSeconds 600
...
foreach ($invalidDeadlineCase in @(
    ...
    [pscustomobject]@{ Qualification = $false; Component = "MacOSPlatformContract"; DeadlineSeconds = 601; Expected = "Promotion component 'MacOSPlatformContract' requires the exact 600-second watchdog deadline" }
...
Assert-True -Condition ($watchdogScript.IndexOf('[ValidateSet("Full", "Solution", "StaticContracts", "NestedProcess", "MacOSPlatformContract")]', [StringComparison]::Ordinal) -ge 0) -Message "The watchdog must expose explicit component modes for the hosted fan-out."
...
Assert-True -Condition ($watchdogScript.IndexOf('"MacOSPlatformContract" { "macos-platform-contract"; break }', [StringComparison]::Ordinal) -ge 0) -Message "The watchdog must map the macOS component to the exact lowercase completion-marker identity."
...
Assert-True -Condition ($qualificationScript.IndexOf('Add-QualificationPhase -Name "format-changed" -FileName "dotnet" -Arguments $formatArguments -TimeoutSeconds 150 -EstimatedDurationSeconds 120 -Weight $qualificationCpuBoundWeight -ResourceClass "CpuBound"', [StringComparison]::Ordinal) -ge 0) -Message "Changed-file formatting must retain its measured Windows-contended estimate, child bound, and CPU posture."
...
Assert-True -Condition ($workflow.IndexOf('runs-on: macos-15', [StringComparison]::Ordinal) -ge 0 -and $workflow.IndexOf('./scripts/verify-with-watchdog.ps1 -Configuration Release -DeadlineSeconds 600 -VerificationComponent MacOSPlatformContract', [StringComparison]::Ordinal) -ge 0) -Message "Promotion must require the ARM64 macOS platform component under the same bounded watchdog transport."
Assert-True -Condition ($workflow.IndexOf('timeout-minutes: 15', $workflow.IndexOf('verify-macos-platform-contract:', [StringComparison]::Ordinal), [StringComparison]::Ordinal) -ge 0) -Message "The macOS component job must retain at least two minutes of setup and upload headroom around its aggregate 600-second verifier budget."
