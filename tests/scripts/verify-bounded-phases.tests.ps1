...
Assert-Contains -Actual $watchdogScript -Expected '[ValidateSet("Full", "Solution", "StaticContracts", "NestedProcess", "MacOSPlatformContract")]' -Message "The external watchdog must expose explicit full, three Windows, and macOS platform-contract component modes."
...
$macOSPlatformContractScript = Get-Content -LiteralPath (Join-Path $repoRoot "scripts\verify-macos-platform-contract.ps1") -Raw
Assert-Contains -Actual $macOSPlatformContractScript -Expected '[Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([Runtime.InteropServices.OSPlatform]::OSX)' -Message "The macOS platform component must reject non-macOS hosts."
Assert-Contains -Actual $macOSPlatformContractScript -Expected '[Runtime.InteropServices.RuntimeInformation]::OSArchitecture -ne [Runtime.InteropServices.Architecture]::Arm64' -Message "The macOS platform component must authenticate the runner architecture."
Assert-Contains -Actual $macOSPlatformContractScript -Expected '[Runtime.InteropServices.RuntimeInformation]::ProcessArchitecture -ne [Runtime.InteropServices.Architecture]::Arm64' -Message "The macOS platform component must authenticate its process/runtime architecture."
Assert-Contains -Actual $macOSPlatformContractScript -Expected '[int]$AggregateDeadlineSeconds = 600' -Message "The macOS platform component must retain one aggregate 600-second budget."
Assert-Contains -Actual $macOSPlatformContractScript -Expected '$remaining = [TimeSpan]::FromSeconds($AggregateDeadlineSeconds) - $Stopwatch.Elapsed' -Message "The macOS platform component must derive each assembly allowance from the shared aggregate budget."
Assert-Contains -Actual $macOSPlatformContractScript -Expected 'Invoke-MacOSPlatformContractPhase "build-macos-platform-contract" "dotnet"' -Message "The macOS component must keep its full Release build inside the aggregate watchdog child."
Assert-Contains -Actual $macOSPlatformContractScript -Expected 'Invoke-MacOSPlatformContractDiscovery' -Message "The macOS component must bind selected facts to discovery before execution."
Assert-Contains -Actual $macOSPlatformContractScript -Expected 'DtdProcessing = [Xml.DtdProcessing]::Prohibit' -Message "The macOS component must reject DTD-bearing TRX evidence."
Assert-Contains -Actual $macOSPlatformContractScript -Expected 'Read-MacOSPlatformContractResults' -Message "The macOS component must derive its result document from raw TRX evidence."
Assert-True -Condition ([regex]::Matches($macOSPlatformContractScript, 'Fact = "EmbodySense\.').Count -eq 15) -Message "The macOS platform component must retain exactly fifteen explicit selected facts."
Assert-True -Condition ([regex]::Matches($macOSPlatformContractScript, 'macos-platform-contract\.trx').Count -eq 1) -Message "The macOS platform component must materialize one shared logger literal once per distinct assembly result directory."
Assert-Contains -Actual $verifyScript -Expected 'Write-VerificationComponentEvidence -Component "macos-platform-contract" -ManifestPaths $macOSManifestPaths -LaneCount 6 -InventoryComplete $true' -Message "The macOS component must reuse the canonical component receipt while declaring only its inventory subset complete."
Assert-Contains -Actual $verifyScript -Expected 'VERIFY_COMPLETE schema_version=1 component=macos-platform-contract status=passed' -Message "The macOS component must emit the exact lowercase completion marker through the external watchdog."
...
Assert-True -Condition ($verifyWorkflow.IndexOf("name: Build macOS platform contract", [StringComparison]::Ordinal) -lt 0) -Message "The macOS build must not sit outside the component's aggregate watchdog."
