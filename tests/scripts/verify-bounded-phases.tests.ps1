...
Assert-Contains -Actual $admissionStoreTest -Expected '"writer" => false' -Message "Successful writers must remain distinct from intentionally terminated child modes."
Assert-Contains -Actual $admissionStoreTest -Expected 'AddUninstrumentedVstestArguments(startInfo, typeof(GovernedLoopAdmissionStoreTests).Assembly.Location, CrossProcessHostTestName)' -Message "Successful cross-process writers must execute the exact xUnit worker without a redundant child collector."
...
$frontierStoreTest = Get-Content -LiteralPath (Join-Path $repoRoot "tests/EmbodySense.Core.Persistence.Tests/Loops/CustomLoopFrontierStoreTests.cs") -Raw
...
Assert-Contains -Actual $coverageEquivalenceScript -Expected 'Get-VerificationCoverageLaneBindings -TestProjects $TestProjects' -Message "Explicit ownership evidence must derive its lane inventory from the checked-in shard map."
...
Assert-True -Condition ($coverageEquivalenceCollectionScript.IndexOf('$LASTEXITCODE', [StringComparison]::Ordinal) -lt 0) -Message "In-process PowerShell evidence scripts must propagate terminating failures directly instead of reading an undefined native-process exit code."
...
Assert-Contains -Actual $admissionStoreTest -Expected 'private const int ChildReadinessTimeoutSeconds = 60;' -Message "Covered child VSTest startup must retain bounded Windows contention headroom without weakening the verifier deadline."
Assert-Contains -Actual $admissionStoreTest -Expected 'private const int GateReleaseMarginSeconds = 15;' -Message "The parent must retain a separate bounded margin to release already-ready children."
Assert-Contains -Actual $admissionStoreTest -Expected 'private const int GateTimeoutSeconds = ChildReadinessTimeoutSeconds + GateReleaseMarginSeconds;' -Message "An early child must remain alive for the complete sibling-readiness window plus the bounded parent release margin."
Assert-Contains -Actual $admissionStoreTest -Expected 'Early_child_gate_wait_remains_open_past_the_complete_sibling_readiness_window' -Message "A deterministic public test must prove an early child's gate wait stays live beyond sibling readiness."
Assert-Contains -Actual $admissionStoreTest -Expected 'Assert.True(GateTimeoutSeconds > ChildReadinessTimeoutSeconds);' -Message "The gate protocol regression must prove its production timeout exceeds the complete readiness bound."
Assert-Contains -Actual $admissionStoreTest -Expected 'await WaitForChildrenReadyAsync(first, firstReady, second, secondReady);' -Message "The two-writer race must observe both child processes while waiting for readiness."
Assert-Contains -Actual $admissionStoreTest -Expected 'if (first.HasExited || second.HasExited)' -Message "A child exit must fail readiness immediately instead of consuming the full bound."
Assert-Contains -Actual $admissionStoreTest -Expected 'process.StandardOutput.ReadToEndAsync()' -Message "Child-readiness failure evidence must retain captured standard output."
Assert-Contains -Actual $admissionStoreTest -Expected 'process.StandardError.ReadToEndAsync()' -Message "Child-readiness failure evidence must retain captured standard error."
Assert-Contains -Actual $admissionStoreTest -Expected 'process.Kill(entireProcessTree: true);' -Message "A readiness timeout must terminate the covered descendant tree before failing."
Assert-Contains -Actual $admissionStoreTest -Expected 'catch (InvalidOperationException) when (process.HasExited)' -Message "Child cleanup must tolerate only the benign exit-between-observation-and-kill race."
Assert-Contains -Actual $admissionStoreTest -Expected 'private const int MaximumChildEvidenceCharacters = 8_192;' -Message "Captured child output must retain a fixed evidence bound."
Assert-Contains -Actual $admissionStoreTest -Expected '"<truncated>" + evidence[^MaximumChildEvidenceCharacters..]' -Message "Oversized child evidence must retain its bounded diagnostic tail."
...
Assert-True -Condition ($verifyScript.IndexOf('@("format", "style", "EmbodySense.sln"', [StringComparison]::Ordinal) -lt 0) -Message "The verifier must not repeat the solution load through a separate style process."
