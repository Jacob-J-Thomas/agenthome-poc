...
Assert-Contains -Actual $verifyWorkflow -Expected 'name: verification-solution-receipt-${{ github.run_attempt }}' -Message "The solution child must publish a small attempt-bound receipt rather than using the full diagnostics artifact for fan-in."
Assert-Contains -Actual $verifyWorkflow -Expected 'name: verification-contract-receipt-${{ github.run_attempt }}' -Message "The static child must publish a small attempt-bound receipt rather than using the full diagnostics artifact for fan-in."
Assert-Contains -Actual $verifyWorkflow -Expected 'pattern: verification-solution-receipt-*' -Message "The protected fan-in must discover retained successful solution receipts across this workflow run's attempts."
Assert-Contains -Actual $verifyWorkflow -Expected 'pattern: verification-contract-receipt-*' -Message "The protected fan-in must discover retained successful static receipts across this workflow run's attempts."
Assert-Contains -Actual $verifyWorkflow -Expected 'merge-multiple: false' -Message "Each component's receipt artifacts must remain isolated so their authenticated attempt identities cannot be mixed."
$fanInJobText = $verifyWorkflow.Substring($fanInJobIndex)
Assert-True -Condition ($fanInJobText.IndexOf('name: verification-solution-receipt-${{ github.run_attempt }}', [StringComparison]::Ordinal) -lt 0 -and $fanInJobText.IndexOf('name: verification-contract-receipt-${{ github.run_attempt }}', [StringComparison]::Ordinal) -lt 0) -Message "The protected fan-in must not request only the current-attempt receipt name."
Assert-True -Condition ($fanInJobText.IndexOf("verification-solution-diagnostics-", [StringComparison]::Ordinal) -lt 0 -and $fanInJobText.IndexOf("verification-contract-diagnostics-", [StringComparison]::Ordinal) -lt 0) -Message "The protected fan-in must not download broad diagnostic artifacts."
