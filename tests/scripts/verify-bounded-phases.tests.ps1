...
Assert-Contains -Actual $verifyScript -Expected '$startupRemainderDiagnosticsEnvironmentVariable = "EMBODYSENSE_SAMPLE3_STARTUP_DIAGNOSTIC"' -Message "The temporary Startup remainder diagnostic must require its explicit opt-in environment variable."
Assert-Contains -Actual $verifyScript -Expected '$startupRemainderDiagnosticLaneName = "EmbodySense.Core.Startup.Tests-remainder"' -Message "The temporary diagnostics must identify only the exact Startup remainder lane."
Assert-Contains -Actual $verifyScript -Expected '$isStartupRemainderDiagnosticLane = $startupRemainderDiagnosticsEnabled -and $Lane.Name -ceq $startupRemainderDiagnosticLaneName' -Message "Diagnostic arguments must remain scoped to the exact Startup remainder lane."
Assert-Contains -Actual $verifyScript -Expected '"--Logger:console;verbosity=$consoleVerbosity"' -Message "The diagnostic lane must retain completed-test output while default lanes remain minimal."
Assert-Contains -Actual $verifyScript -Expected '$Lane.Environment.VSTEST_DUMP_PATH = $hangDumpsPath' -Message "The diagnostic lane must route stock VSTest dumps beneath its result directory."
Assert-Contains -Actual $verifyScript -Expected '"--Diag:$(Join-Path $diagnosticsPath ''vstest.diag.log'');tracelevel=verbose"' -Message "The diagnostic lane must enable verbose stock VSTest traces."
Assert-Contains -Actual $verifyScript -Expected '"/Blame:CollectHangDump;TestTimeout=90s;HangDumpType=mini"' -Message "The diagnostic lane must request a mini dump after 90 seconds without test progress."
Assert-Contains -Actual $verifyScript -Expected 'authority=diagnostic-only' -Message "Diagnostic output must not represent acceptance evidence."
Assert-Contains -Actual $verifyScript -Expected 'dump_best_effort=true' -Message "Diagnostic output must state that stock hang-dump capture is best effort."
