...
Assert-Contains -Actual $qualificationWorkflow -Expected '-Qualification -BaseCommit ''${{ inputs.base_sha }}'' -HeadCommit ''${{ inputs.head_sha }}'' -Configuration Release -DeadlineSeconds 900' -Message "Qualification must bind the dispatched exact edge under one fifteen-minute watchdog."
Assert-Contains -Actual $qualificationWorkflow -Expected "    timeout-minutes: 17" -Message "Hosted qualification must leave two minutes of setup and diagnostic-upload margin around its fifteen-minute child watchdog."
