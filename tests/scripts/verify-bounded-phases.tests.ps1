...
Assert-Contains -Actual $pullRequestSettings -Expected '<TestSessionTimeout>1200000</TestSessionTimeout>' -Message "Pull-request test sessions must retain enough bounded time for the Persistence coverage suite."
Assert-Contains -Actual $verifyScript -Expected '$coveragePhaseTimeoutSeconds = if ($_.Name -eq "EmbodySense.Core.Persistence.Tests.csproj") { 1260 } else { 900 }' -Message "Only Persistence coverage receives the extended phase timeout."
