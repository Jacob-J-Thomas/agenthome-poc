...
Assert-Contains -Actual $verifyScript -Expected '$preflightNestedProcessContractWeight = Get-VerificationPreflightNestedProcessContractWeight -ResourceCapacity $preflightResourceCapacity' -Message "Nested-process script contracts must use the behavior-tested two-unit preflight weight."
Assert-Contains -Actual $verifyScript -Expected '$preflightMaximumProcessHeavyWorkers = [Math]::Min(2, $preflightMaximumWorkers)' -Message "Each bounded schedule must admit no more than two process-heavy preflight workers."
...
Assert-Contains -Actual $scheduleScript -Expected 'return [Math]::Min(2, $ResourceCapacity)' -Message "Nested-process script contracts must reserve two physical units where available."
...
try {
    foreach ($admissionCase in @(
        ...
        Assert-True -Condition ($nestedProcessContractWeight -eq [Math]::Min(2, $admissionCase.Capacity)) -Message "Nested-process contract capacity $($admissionCase.Capacity) must reserve at most two preflight units."
    }
}
finally {
    Reset-VerificationParallelPhaseState
...
'@, [Text.UTF8Encoding]::new($false))
    ...
    Reset-VerificationParallelPhaseState
    Add-VerificationParallelPhase -Name "nested-first" -FileName $powerShellExecutable -Arguments (Get-PreflightTimingArguments -ScriptPath $timingScriptPath -Role "nested-first" -SynchronizationRoot $behaviorRoot) -TimeoutSeconds 10 -WorkingDirectory $repoRoot -OutputPath $nestedFirstOutputPath -EstimatedDurationSeconds 60 -Weight 2 -ResourceClass "ProcessHeavy"
    Add-VerificationParallelPhase -Name "nested-second" -FileName $powerShellExecutable -Arguments (Get-PreflightTimingArguments -ScriptPath $timingScriptPath -Role "nested-second" -SynchronizationRoot $behaviorRoot) -TimeoutSeconds 10 -WorkingDirectory $repoRoot -OutputPath $nestedSecondOutputPath -EstimatedDurationSeconds 60 -Weight 2 -ResourceClass "ProcessHeavy"
    $postBuildResults = @(Invoke-VerificationParallelPhases -MaximumWorkers 4 -MaximumResourceCapacity 4 -MaximumProcessHeavyWorkers 2)
    ...
    Assert-True -Condition ($nestedFirstTiming.Start -lt $nestedSecondTiming.End -and $nestedSecondTiming.Start -lt $nestedFirstTiming.End) -Message "Disjoint nested-process contracts must execute two-wide instead of extending the serialized preflight tail."
    foreach ($nestedTiming in @($nestedFirstTiming, $nestedSecondTiming)) {
        foreach ($otherTiming in $nonNestedTimings) {
            Assert-True -Condition ($nestedTiming.End -le $otherTiming.Start -or $otherTiming.End -le $nestedTiming.Start) -Message "The post-build nested-process wave must not overlap build, frontend, or coverage work."
        }
    }
}
finally {
