...
Assert-Contains -Actual $verifyScript -Expected 'Invoke-VerificationParallelPhases -MaximumWorkers $hardwareBoundedResourceCapacity -MaximumResourceCapacity $hardwareBoundedResourceCapacity -MaximumBackfillsBeforeReservation 4 | Out-Null' -Message "The preflight must aggregate every child through the fail-closed scheduler without exceeding physical hosted-runner capacity while filling otherwise idle capacity with at most four backfills."
