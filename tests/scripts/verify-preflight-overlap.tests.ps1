...
Assert-Contains -Actual $verifyScript -Expected '$contractTimeoutSeconds = if ($contractScript -ceq "verify-watchdog.tests.ps1") { 120 } else { 90 }' -Message "Only the source-heavy watchdog contract may use the measured ordinary-contract headroom."
...
foreach ($ordinaryContract in @("verify-bounded-phases.tests.ps1", "verify-test-inventory.tests.ps1")) {
...
Assert-OccurrenceCount -Actual $verifyScript -Expected '"verify-watchdog.tests.ps1"' -ExpectedCount 3 -Message "The source-heavy watchdog contract must have one execution-manifest entry, one overlap-classification entry, and one bounded headroom classification."
