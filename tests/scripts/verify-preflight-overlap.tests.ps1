...
Assert-Contains -Actual $scheduleScript -Expected 'function Get-VerificationPreflightNestedProcessContractScripts {' -Message "Nested-process script ownership must have one shared platform-aware manifest."
...
Assert-Contains -Actual $verifyScript -Expected 'Add-VerificationParallelPhase -Name "contract-verify-coverage.tests" -FileName $powerShellExecutable -Arguments $coverageContractArguments -TimeoutSeconds 120 -WorkingDirectory $repoRoot -OutputPath (Join-Path $verificationLogsPath "verify-coverage.tests.ps1.log") -EstimatedDurationSeconds 75 -Weight $preflightCoverageContractWeight -ResourceClass "ProcessHeavy"' -Message "The coverage contract must retain its measured process-heavy profile and bounded headroom inside build overlap."
Assert-Contains -Actual $verifyScript -Expected 'if ($preflightNestedProcessContractScripts -ccontains $contractScript) {' -Message "Known descendant-heavy script contracts must be classified explicitly before fail-closed rejection."
...
Assert-Contains -Actual $verifyScript -Expected '$contractTimeoutSeconds = if ($contractScript -ceq "verify-watchdog.tests.ps1") { 120 } else { 90 }' -Message "The source-heavy watchdog contract must retain its measured ordinary-contract headroom."
Assert-Contains -Actual $verifyScript -Expected 'Add-VerificationParallelPhase -Name "contract-$([IO.Path]::GetFileNameWithoutExtension($contractScript))" -FileName $powerShellExecutable -Arguments $contractArguments -TimeoutSeconds $contractTimeoutSeconds -WorkingDirectory $repoRoot -OutputPath (Join-Path $verificationLogsPath "$contractScript.log") -EstimatedDurationSeconds 35 -Weight 1 -ResourceClass "Ordinary"' -Message "Independent script contracts must retain their bounded Ordinary profile."
Assert-OccurrenceCount -Actual $verifyScript -Expected '            "verify-coverage.tests.ps1",' -ExpectedCount 1 -Message "The coverage contract must appear exactly once in the full execution manifest."
foreach ($nestedProcessContract in @("verify-preflight-overlap.tests.ps1", "verify-parallel.tests.ps1")) {
    Assert-OccurrenceCount -Actual $verifyScript -Expected "`"$nestedProcessContract`"" -ExpectedCount 2 -Message "Each descendant-heavy contract must appear in the full and static execution manifests."
}
foreach ($ordinaryContract in @("verify-bounded-phases.tests.ps1", "verify-test-inventory.tests.ps1", "verify-promotion-fan-in.tests.ps1")) {
    Assert-OccurrenceCount -Actual $verifyScript -Expected "`"$ordinaryContract`"" -ExpectedCount 3 -Message "Each build-safe contract must appear in the full and static manifests plus its overlap classification."
}
Assert-OccurrenceCount -Actual $verifyScript -Expected '"verify-watchdog.tests.ps1"' -ExpectedCount 4 -Message "The source-heavy watchdog contract must retain full/static manifests, overlap classification, and bounded headroom classification."
Assert-OccurrenceCount -Actual $verifyScript -Expected '"verify-sdk-diagnostics.tests.ps1"' -ExpectedCount 2 -Message "The Windows process-tree diagnostic contract must be present in both hosted component manifests."
Assert-True -Condition ((@(Get-VerificationPreflightNestedProcessContractScripts).Count -eq 2) -and (@(Get-VerificationPreflightNestedProcessContractScripts -RunningOnWindows).Count -eq 3)) -Message "The shared nested-process manifest must retain two portable contracts and add the Windows SDK diagnostic contract only on Windows."
...
Assert-Contains -Actual $verifyScript -Expected 'Add-ProfiledRequiredGatePhase -Name "format-whitespace" -FileName "dotnet" -Arguments @("format", "whitespace", "EmbodySense.sln", "--verify-no-changes", "--no-restore", "--verbosity", "minimal") -OutputPath (Join-Path $verificationLogsPath "format-whitespace.log")' -Message "The authoritative whitespace gate must source its timeout from the checked-in required-gate profile after immutable test outputs exist."
Assert-Contains -Actual $verifyScript -Expected 'Add-ProfiledRequiredGatePhase -Name "format-naming-style" -FileName "dotnet" -Arguments @("format", "style", "EmbodySense.sln", "--verify-no-changes", "--no-restore", "--severity", "warn", "--diagnostics", "IDE1006", "--verbosity", "minimal") -OutputPath (Join-Path $verificationLogsPath "format-naming-style.log")' -Message "The authoritative naming/style gate must source its timeout from the checked-in required-gate profile after immutable test outputs exist."
...
Assert-Contains -Actual $frontendScript -Expected 'Invoke-NpmVerificationPhase -Name "npm-ci" -NpmArguments @("ci", "--include=dev", "--no-audit", "--no-fund")' -Message "The composed phase must install the exact development dependency set without nonessential audit or funding registry calls."
Assert-NotContains -Actual $frontendScript -Expected 'Invoke-NpmVerificationPhase -Name "npm-ci" -NpmArguments @("ci", "--include=dev")' -Message "The required install must not restore nonessential audit and funding network calls."
Assert-NotContains -Actual $frontendScript -Expected '"--offline"' -Message "The required install must retain cold-cache registry validation."
Assert-NotContains -Actual $frontendScript -Expected '"--ignore-scripts"' -Message "The required install must not weaken package lifecycle semantics."
...
'@, [Text.UTF8Encoding]::new($false))
    ...
    Assert-True -Condition ($postBuildResults.Count -eq 2 -and @($postBuildResults | Where-Object { $_.ExitCode -ne 0 }).Count -eq 0) -Message "The bounded post-build contract schedule must complete every declared phase exactly once and successfully."
    Assert-True -Condition (@($postBuildResults | Select-Object -ExpandProperty Name | Sort-Object) -join "," -ceq "nested-first,nested-second") -Message "Post-build contract execution must contain only the exact descendant-heavy phase set."
...
$fullVerificationIndex = $verifyScript.IndexOf('$normalPullRequestVerification', [StringComparison]::Ordinal)
$nestedAdmissionIndex = $verifyScript.IndexOf('foreach ($contractScript in $contractScripts) {', $fullVerificationIndex, [StringComparison]::Ordinal)
...
Assert-True -Condition ($buildIndex -lt $buildInvocationIndex -and $frontendIndex -lt $buildInvocationIndex -and $coverageIndex -lt $buildInvocationIndex) -Message "Build, frontend, and dependency-ordered coverage must enter the same bounded overlap schedule."
...
Assert-True -Condition ($buildCompletionIndex -lt $nestedAdmissionIndex) -Message "Descendant-heavy contracts may be admitted only after build overlap succeeds."
