...
Assert-Contains -Actual $verifyScript -Expected '-EstimatedDurationSeconds 35 -Weight 1 -ResourceClass "Ordinary"' -Message "Every independent script contract must declare its bounded preflight resource cost."
Assert-Contains -Actual $verifyScript -Expected 'kind=pull-request-preflight phases=$($script:VerificationParallelPhases.Count) maximum_workers=$MaximumTestWorkers maximum_resource_capacity=$hardwareBoundedResourceCapacity' -Message "The overlapping preflight must publish distinct worker and resource limits."
