...
$requiredGateVirtualSchedule = Get-VirtualVerificationSchedule -Profiles $requiredGateProfiles -MaximumWorkers 3 -MaximumResourceCapacity 12 -MaximumProcessHeavyWorkers 3 -MaximumCpuBoundWorkers 1
...
Assert-True -Condition ($initialResourceCapacity -eq 9) -Message "Three assembly-wide phases must occupy nine logical units without exceeding three actual workers."
Assert-True -Condition ($requiredGateVirtualSchedule.Starts["format-naming-style"] -gt 0) -Message "The longer format gate must wait for an outer process slot instead of admitting a fourth process."
...
foreach ($formatGateName in @("format-naming-style", "format-whitespace")) {
    Assert-True -Condition ($requiredGateVirtualSchedule.Starts[$formatGateName] -lt 390) -Message "Format gate '$formatGateName' must execute before the final assembly-wide gate completes."
}
