...
public sealed partial class BrowserFlowTests
{
    ...
    public async Task Human_review_browser_rejects_an_old_session_cookie_after_real_process_restart()
    {
        ...
        try
        {
            ...
            var staleStatus = await browser.EvaluateInt32Async($"(async () => {{ const response = await fetch('/api/human-reviews/{Uri.EscapeDataString(runId)}', {{ cache: 'no-store', credentials: 'same-origin' }}); return response.status; }})()");
