...
public sealed partial class AgentRuntimeHumanInputLifecycleTests
{
    ...
    public async Task Human_input_facade_resolves_remind_grant_evidence_and_maps_missing_grant_without_leaking_payload()
    {
        ...
        var create = AgentRuntimeFactoryTests.CreateFreshHumanInputMutation(workspace.RootPath, "request-grant-coverage", "version-grant-coverage", "create-grant-coverage", HumanInputRequestStoreTestData.HashA);
        ...
        var persistedRemind = AgentRuntimeFactoryTests.CreateDurableLifecycleReplayMutation(
    ...
    public async Task Human_input_facade_remind_resolves_the_create_grant_reference_when_no_remind_evidence_exists()
    {
        ...
        var paths = new WorkspacePaths(workspace.RootPath);
        var store = new HumanInputRequestStore(paths, new FileCapabilityCatalogTrustProvider(workspace.ServerStatePath));
        var create = AgentRuntimeFactoryTests.CreateFreshHumanInputMutation(workspace.RootPath, "request-ambiguous-grant", "version-ambiguous-grant", "create-ambiguous-grant", HumanInputRequestStoreTestData.HashB);
        Assert.Equal(HumanInputRequestLifecycleStoreCommitStatus.Committed, (await store.CommitAsync(create)).Status);
        var request = Assert.IsType<HumanInputRequest>(create.RequestToAppend);
    ...
    public async Task Human_input_facade_remind_returns_conflict_before_grant_resolution_for_stale_head()
    {
        ...
        var paths = new WorkspacePaths(workspace.RootPath);
        var store = new HumanInputRequestStore(paths, new FileCapabilityCatalogTrustProvider(workspace.ServerStatePath));
        var create = AgentRuntimeFactoryTests.CreateFreshHumanInputMutation(workspace.RootPath, "request-stale-grant", "version-stale-grant", "create-stale-grant", HumanInputRequestStoreTestData.HashC);
        Assert.Equal(HumanInputRequestLifecycleStoreCommitStatus.Committed, (await store.CommitAsync(create)).Status);
        var request = Assert.IsType<HumanInputRequest>(create.RequestToAppend);
        var head = Assert.IsType<HumanInputRequestLifecycleHead>(create.PrimaryHeadToWrite);
        var provider = new HumanInputRuntimeFacadeTestAuthorityProvider();
        ...
        HumanInputOperationStatus expectedStatus)
    {
        ...
        var paths = new WorkspacePaths(workspace.RootPath);
        var store = new HumanInputRequestStore(paths, new FileCapabilityCatalogTrustProvider(workspace.ServerStatePath));
        var create = AgentRuntimeFactoryTests.CreateFreshHumanInputMutation(workspace.RootPath, "request-authority-coverage", "version-authority-coverage", "create-authority-coverage", HumanInputRequestStoreTestData.HashA);
        Assert.Equal(HumanInputRequestLifecycleStoreCommitStatus.Committed, (await store.CommitAsync(create)).Status);
        var request = Assert.IsType<HumanInputRequest>(create.RequestToAppend);
        var head = Assert.IsType<HumanInputRequestLifecycleHead>(create.PrimaryHeadToWrite);
