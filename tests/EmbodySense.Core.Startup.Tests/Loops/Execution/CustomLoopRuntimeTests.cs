...
public sealed class CustomLoopRuntimeTests
{
    ...
    public async Task Public_runtime_admits_executes_publishes_and_exposes_inspectable_artifacts_without_changing_default_turns()
    {
        ...
        Assert.Equal("WorkspaceRoleFile", roleSource.Provenance);
        ...
        var soulSource = Assert.Single(response.Run.Context.SourceManifest, source => source.SourceId == "soul");
        Assert.Equal("AgentIdentityFile", soulSource.Provenance);
        Assert.Contains("durable identity instruction source", soulSource.Content, StringComparison.Ordinal);
