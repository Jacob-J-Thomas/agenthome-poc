...
public sealed class WorkspaceConfigurationReaderTests
{
    ...
    public async Task ReadAsync_returns_initialized_workspace_configuration_details()
    {
        ...
        Assert.Contains(snapshot.Documents, document => document.Name == "Role instructions" && document.Category == "Role" && document.Exists && document.Content.Contains("Workspace role", StringComparison.Ordinal));
    ...
    public async Task ReadAsync_reports_missing_workspace_configuration_without_throwing()
    {
        ...
        Assert.Contains(snapshot.Documents, document => document.Name == "Role instructions" && !document.Exists && document.Path.EndsWith("ROLE.md", StringComparison.Ordinal));
    ...
    public async Task ReadAsync_caps_sensitive_snapshot_content()
    {
        ...
        var roleInstructions = Assert.Single(snapshot.Documents, document => document.Name == "Role instructions");
        Assert.Contains("api_key= [redacted]", roleInstructions.Content);
        Assert.Contains("[truncated after", roleInstructions.Content);
