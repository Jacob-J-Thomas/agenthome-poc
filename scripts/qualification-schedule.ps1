...
$script:QualificationTestScheduleProfiles = @(
    # https://github.com/Jacob-J-Thomas/agenthome-poc/issues/610: reserve six of eight units on the four-worker qualification posture for full Persistence. Run the two Windows-dominant full suites in the declared protected order because hosted run 33033802308 proved their concurrent pair unsafe. Broad run 33047472480 proved the former 270-second Persistence limit too narrow, while promotion run 33032062561 measured Startup's canonical lanes at 589.354 seconds; the 300/690 child limits preserve the existing 1,630-second protected cap and fifty-second watchdog reserve.
    [pscustomobject]@{ ProjectName = "EmbodySense.Core.Persistence.Tests"; EstimatedDurationSeconds = 270; TimeoutSeconds = 300; Weight = 6; ResourceClass = "ProcessHeavy"; Isolation = "Exclusive"; ExclusiveOrder = 1 }
    [pscustomobject]@{ ProjectName = "EmbodySense.Core.Startup.Tests"; EstimatedDurationSeconds = 600; TimeoutSeconds = 690; Weight = 3; ResourceClass = "ProcessHeavy"; Isolation = "Exclusive"; ExclusiveOrder = 2 }
