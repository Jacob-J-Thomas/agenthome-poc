...
| LIFE-010 | Wave one supports exactly one active custom-loop execution host per workspace. A conflicting process fails closed for dispatch and automatic continuation, but may inspect durable lifecycle state and issue an authenticated, bounded cancellation request to the published owner generation. This cancellation exception does not provide automatic failover or resume execution. |
