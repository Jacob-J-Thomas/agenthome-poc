...
| LIM-016 | Governed tool requests | 5 per inference attempt and 30 per run, plus one model-visible over-limit denial and one bounded repeated-request integrity record | The first excess request is denied, traced, audited, and returned without execution. Any later request is not actuated or returned to the model; retain its exact honest ordinal, correlation, command, arguments, resolved target, and authority once, then fail the attempt. |
...
| RUN-019 | An invocation receipt begins with only the bounded caller envelope, then atomically binds either the stable logical-conversation identity plus a timestamp-independent hash of captured context before admission, or the stable conversation identity plus an exact pre-dispatch terminal intent. A captured binding may terminalize as `NotFound` if the definition disappears during admission, and a definition-read failure retains an already captured binding. A pending `NotFound`, `WorkspaceExecutionBusy`, or `Invalid` terminal binding can only finish that no-dispatch outcome after interruption and never upgrades into admission. Completed replay requires the same logical conversation and verifies any referenced run using outcome-specific relationships. Receipts retain hashes, identifiers, and bounded status detail, never the invocation prompt or captured context content. |
...
| LIFE-004 | Cancellation propagates to provider, approval wait, tool execution, and traversal where supported. The workspace host identifies the active attempt owner and accepts same-process or cross-process cancellation through a generation-scoped, capability-authenticated local IPC request. Receipts distinguish confirmed provider interruption, signal delivery without confirmation, and no active attempt; owner exit or a bounded routing timeout leaves the idempotent receipt pending for retry. No later step starts after `CancelRequested`. Once an outcome may exist, mandatory trace/audit/checkpoint writes use the independent bounded integrity token from LIM-018 and are not cancelled by the caller token. |
...
| LIFE-009 | Durable expected-state/version transitions reject duplicate Invoke, Resume, Pause, Cancel, and terminal races deterministically. Each pending lifecycle-control receipt holds a cross-process execution lease and records its owner generation. A retry fails closed while that lease is live; successful lock reacquisition after return or process exit proves orphaning and permits only the same explicit request to re-drive the pre-transition window. An in-memory semaphore alone is not the persistence contract, and receipt recovery never triggers automatic loop continuation. |
| LIFE-010 | Wave one permits exactly one active custom-loop execution host per workspace. A conflicting process fails closed for dispatch but may inspect durable run evidence and issue governed lifecycle controls. Cross-process behavior is limited to the authenticated cancellation route and the bounded receipt-ownership proof in LIFE-009; neither mechanism provides automatic failover or continuation. |
| LIFE-011 | At most one nonterminal run exists per loop and at most one custom loop actively executes in the workspace. Before `Admitted -> Running` or `Paused -> Running`, atomically acquire the cross-process gate and publish its bounded cancellation-owner generation. If unavailable, Invoke returns the idempotent `workspace_execution_busy` rejection and Resume remains Paused; neither request queues, starts deadline accounting, mutates lifecycle, nor rebinds approval ownership. Custom runs use isolated run-scoped provider transport, ToolBroker, approval owner, and cancellation state rather than the default conversation loop's objects. Paused runs release the custom gate; ordinary chat behavior/availability is not globally serialized behind it. |
...
- Checkpoints, lifecycle transitions, each bounded tool request, server-resolved target, authority/permission/approval decisions, exact canonical tool result returned to the model, its separately retained complete-response manifest/hash reference, the exact non-actuating request retained when a model repeats after the visible over-limit denial, failures, and final result.
...
Trace storage is bounded by LIM-019 and never pruned automatically. Governed requests retain reservation-time authority first, then refreshed pre-actuation authority with the governance and outcome phases. The production maximum-bounded-shape runner/codec fixture includes 30 allowed governed requests with distinct maximum-size refreshed authority plus the visible 31st over-limit denial and encodes to 15,296,441 bytes, which leaves 432,199 bytes beneath the 15 MiB reservation target and 1,480,775 bytes beneath the 16 MiB hard cap while retaining every required event and exact bounded payload. A terminal trace without an integrity warning continues accounting its reserved 8,192-byte warning slot, for 15,304,633 accounted bytes and 424,007 bytes of remaining 15 MiB headroom at that maximum shape. While the run remains nonterminal, the permanent reserve also holds capacity for the one exact non-actuating repeated-request integrity record; that record does not increment model-visible checkpoint usage. The Web UI shows current count/bytes and reserved capacity. An authenticated user may explicitly delete a terminal run's sensitive content using its expected trace hash and a confirmation operation ID. Deletion retains a small immutable tombstone containing run/loop identity, terminal status, definition/trace hashes, timestamps, deletion actor/surface, and audit correlation; it never rewrites the historical audit log. Tombstones and deletion-operation receipts have separately named and visible limits. The last 10,000 receipt slots are reserved for requests that can replace a live terminal trace. Exact receipts are retained without automatic cleanup or compaction, so replay and operation-id conflict detection remain fail closed; exhausting the receipt ledger produces a distinct observable rejection.
