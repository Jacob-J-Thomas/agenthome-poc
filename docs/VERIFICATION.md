...
Every canonical qualification test project and verifier contract has one checked-in measured scheduling profile. The two Windows-dominant suites therefore start first and retain bounded child headroom beneath the eight-minute global deadline, while a new project without a profile fails closed. On a four-worker host, the second wave exposes eight logical units: no more than two three-unit process-heavy lanes can run while up to two checked one-unit process-light lanes backfill the remaining physical slots. A CPU-bound formatter retains three units and therefore cannot join two dominant suites; the physical process ceiling remains four.
