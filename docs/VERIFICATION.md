...
Required gates use six logical resource units and deterministic longest-processing-time-first ordering from checked-in duration estimates. Ordinary isolated lanes consume one unit, CPU-bound format/frontend gates consume two, and helper-process-heavy lanes consume three, so at most two process-heavy phases overlap; missing profiles, oversized weights, and underweighted resource classes fail before execution. Script contracts, discovery, and reconciliation remain hardware-bounded by the smaller of eight requested units and the host's logical-processor count. Startup loop execution is partitioned into custom-runtime, governed-runtime, and explicit remainder lanes so its two external-process-heavy families cannot serialize the entire assembly. Every test assembly is discovered once as a canonical selection and again through its declared lanes. The partition reconciler uses xUnit's stable test-case unique ID to require exact, disjoint, nonempty lane coverage; execution reconciliation then requires every VSTest test-case ID in the expected inventory and rejects missing, unexpected, failed, or cross-report duplicate executions. Dynamic data may produce multiple execution rows for one discovered case only within its one admitted lane report.
