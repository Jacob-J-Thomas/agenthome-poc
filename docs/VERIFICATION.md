...
Qualification uses three dependency-safe scheduler waves. Independent build and frontend prerequisites share the first bounded wave; after immutable prerequisites are proved, selected tests, shared verifier contracts, workflow validation, changed-file formatting, and exact diff-check share the second. The parallel-scheduler contract recursively launches enough real children to saturate the host, so it alone runs in a final exclusive wave instead of racing product suites or weakening its synchronization assertions. Every selected project receives a collision-resistant short temporary root, and one `dotnet format` workspace load preserves both whitespace and IDE1006 diagnostics.
...
Every canonical qualification test project and verifier contract has one checked-in measured duration, timeout, resource class, weight, and isolation profile. The two Windows-dominant suites therefore start first and retain bounded child headroom beneath the eight-minute global deadline, while a new unprofiled project fails closed. On a four-worker host, the shared-work wave exposes eight logical units: no more than two three-unit process-heavy lanes can run while up to two checked one-unit process-light lanes backfill the remaining physical slots. A CPU-bound formatter retains three units and therefore cannot join two dominant suites; the physical process ceiling remains four.
...
The verifier defaults to `Release`; required workflows pass `-Configuration Release` explicitly. `Debug` remains an explicit local opt-in through `-Configuration Debug`. The watchdog accepts the 480-second qualification deadline and the workflow's 600-second promotion deadline, but rejects any larger value. The selected deadline is inclusive: exactly 600 seconds passes, while the first later tick, cancellation, child timeout, nonzero exit, or missing/duplicate terminal marker fails closed. Tool setup and diagnostic upload remain outside the measured child and the workflow job ceiling. The stress workflow always runs its diagnostic-upload step, retains available TRX timing and failure diagnostics for 30 days, and fails if the expected artifacts are absent. Exact filters plus `TreatNoTestsAsError` make removal, renaming, or trait loss fail the job instead of silently producing an empty pass.
...
Qualification has an exact eight-minute limit and owns the development-feedback contract. Promotion is an exhaustive once-per-stable-head certification pass with an inclusive ten-minute limit. `scripts/verify-with-watchdog.ps1` measures either child with `System.Diagnostics.Stopwatch`, terminates its process tree at the selected deadline, captures combined output, labels the evidence as `qualification` or `promotion`, and accepts exactly one terminal record of the form `VERIFY_COMPLETE schema_version=1 status=passed elapsed_seconds=...`. Qualification retains its plan, logs, and selected TRX under `tests/QualificationResults`; promotion retains its complete phase, inventory, coverage, and watchdog evidence under `tests/VerificationResults`. A breach is a verifier incident to investigate, not authority to remove tests, reduce coverage, weaken assertions, or turn certification back into the draft edit loop.
