...
Each lane executes an immutable copy of the exact Release build under `<isolation>/<project>/<lane>/bin/Release/net10.0`, preserving helper-host path semantics. Collectors, results, and process environment are lane-scoped. Each lane also receives a unique absolute `EMBODYSENSE_CAPABILITY_CATALOG_TRUST_ROOT` beneath an ephemeral system-temporary root, so parallel processes and their descendants cannot share default server trust state; that root is outside retained diagnostics and is removed after ordinary completion. Persistence child-process coverage receives a process-scoped immutable source and never mutates the shared build output. After execution, SHA-256 manifests prove the source, canonical copy, and every lane copy unchanged.
