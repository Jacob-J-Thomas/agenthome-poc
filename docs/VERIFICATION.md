...
Exact-head Windows run 31753376066 executed all 5,126 canonical tests as 5,240 passing dynamic rows across all nine lanes and emitted their coverage reports. Final threshold and inventory reconciliation began at 598.289 seconds and the unchanged watchdog terminated it, which is why the CPU-bound format profiles now reserve the two-heavy schedule's remaining capacity rather than admitting a fourth outer process during the compiler-heavy window. The checked-in 480-second virtual bound covers required gates only; exact-head Windows whole-run timing remains the acceptance authority.
