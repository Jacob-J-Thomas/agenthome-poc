...
The Pull Request tier starts with one bounded build-overlap wave. The isolated Release build runs beside the independent ordered frontend install/lint/format/test chain; only the explicitly classified source/temp-only bounded-phase, inventory, and watchdog contracts may backfill capacity as one of those two phases finishes. The process-heavy coverage contract is admitted to that same schedule behind the longer Release-build phase: longest-first ordering plus its three-unit weight prevents it from fitting beside the two-unit build, while it can backfill beside one remaining source-only contract after the build completes. A successful whole-wave boundary is required before the second wave can admit descendant-heavy scheduler contracts or Windows SDK process-tree diagnostics. Those suites use distinct temporary process trees and immutable repository reads, reserve two physical units each, and run at most two-wide; every assertion, timeout, and aggregate fail-closed result remains owned by its original script. Every phase retains explicit weights, class limits, logs, timeouts, and fail-closed aggregate completion.

Required gates use twelve logical resource units but no more than four outer processes on the four-core hosted runner. Deterministic longest-processing-time-first ordering uses checked-in duration estimates. The four internally parallel assembly gates consume three units each, but at most two are admitted together; each VSTest process permits at most two xUnit threads. One combined CPU-bound C# format phase reserves the remaining six units, so ordinary work waits until its single solution load drains instead of oversubscribing the compiler-heavy window. The phase omits a formatter subcommand intentionally: `dotnet format EmbodySense.sln --verify-no-changes --no-restore --severity warn --diagnostics IDE1006 --verbosity minimal` runs whitespace formatting and the explicitly selected IDE1006 style analyzer together. It joins the required schedule only after build-output isolation, discovery, and exact partition reconciliation, allowing it to overlap immutable test execution without racing build artifacts. Missing profiles, oversized weights, underweighted resource classes, and limits above the effective worker ceiling fail before execution. Discovery and reconciliation remain hardware-bounded. The current partition has exactly nine coverage lanes, one per production test assembly, eliminating repeated VSTest startup, deployment, instrumentation, and Cobertura-write overhead without removing tests. Tests that share Startup runtime state, default capability-trust state, ephemeral API-host resources, or process-global state remain serialized through explicit collections; process-global cases disable collection parallelization entirely. Coverage child-process helper contracts take their immutable assembly directory as an explicit test coordinate, keeping those self-tests eligible for the assembly's bounded second xUnit thread without changing child-process isolation or coverage collection. Successful covered child startup has a 60-second readiness bound, fails immediately when either child exits, and captures bounded stdout/stderr evidence. Because both children must be ready before the parent releases the race, an early child waits for that complete sibling-readiness window plus a separately bounded 15-second release margin. Every test assembly is still discovered once as a canonical selection and partitioned through its one declared lane. The partition reconciler uses xUnit's stable test-case unique ID to require exact, disjoint, nonempty lane coverage; execution reconciliation then requires every VSTest test-case ID in the expected inventory and rejects missing, unexpected, failed, or cross-report duplicate executions. Dynamic data may produce multiple execution rows for one discovered case only within its admitted assembly report.
...
Exact-head Windows run 31753376066 executed all 5,126 canonical tests as 5,240 passing dynamic rows across all nine lanes and emitted their coverage reports. Final threshold and inventory reconciliation began at 598.289 seconds and the unchanged watchdog terminated it, which is why the CPU-bound format profile reserves the two-heavy schedule's remaining capacity rather than admitting a fourth outer process during the compiler-heavy window. Exact-head run 31757214113 then proved that the physical second formatter load was still material: naming and whitespace consumed 90.979 and 46.430 seconds. That run also exposed a Windows-contention defect in the successful covered admission-writer race: one child had not published readiness inside the prior 15-second startup bound, so Persistence completed 1,662 of 1,663 tests successfully and failed before Web, Integration, inventory reconciliation, or coverage thresholds could complete. An isolated .NET 10 fixture proved the combined command fails independently for a whitespace-only defect and an IDE1006-only defect. Two alternating full-solution local measurements recorded sequential whitespace-plus-style totals of 49.65 and 34.86 seconds versus combined totals of 37.51 and 34.77 seconds. The warm comparison is effectively equal, while the combined command removes one physical workspace load and retained both logical checks; exact-head Windows remains the performance authority. The checked-in 480-second virtual bound covers required gates only; exact-head Windows whole-run timing remains the acceptance authority.
