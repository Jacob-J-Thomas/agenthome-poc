...
Required gates use eight logical resource units, no more than six child workers on the four-core hosted runner, and deterministic longest-processing-time-first ordering from checked-in duration estimates. Ordinary isolated lanes consume one unit, CPU-bound format/frontend gates consume two, and sustained helper-process-heavy lanes consume three. Independent class limits admit at most two process-heavy phases and one CPU-bound phase even when residual capacity remains; missing profiles, oversized weights, underweighted resource classes, and limits above the effective worker ceiling fail before execution. Script contracts, discovery, and reconciliation remain hardware-bounded by the smaller of eight requested units and the host's logical-processor count. Startup retains dedicated custom and governed runtime lanes; fast Application and Common projects and persistence families using short-lived purpose-built hosts are deliberately consolidated so duplicate Cobertura reports do not dominate the budget. Every test assembly is discovered once as a canonical selection and partitioned through its declared lanes. The partition reconciler uses xUnit's stable test-case unique ID to require exact, disjoint, nonempty lane coverage; execution reconciliation then requires every VSTest test-case ID in the expected inventory and rejects missing, unexpected, failed, or cross-report duplicate executions. Dynamic data may produce multiple execution rows for one discovered case only within its one admitted lane report.
