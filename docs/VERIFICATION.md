...
| --- | --- | --- |
| Qualification | Every current pull-request head, including drafts; `verify-with-watchdog.ps1 -Qualification -BaseCommit <base> -HeadCommit <head> -DeadlineSeconds 300`. A newer commit cancels the superseded qualification run. | An exact merge-base-to-head ownership map selects only affected checks. Product C# changes receive a Release solution build, the full owning project suite, architecture tests, and changed-file formatting. General Common changes run every direct test-project consumer; CLI Command, Application, and Clients changes also run the complete Integration suite; Web changes additionally run the non-browser E2E suite; Startup changes run its complete CLI Command, Web, and Integration consumer suites. Common and Startup consumer-map contracts derive those closures from current project references. Test C# files that directly declare xUnit methods authenticate exactly one filename-matching top-level class from the exact head and run that class with empty selection treated as an error; deleting a test source restores the surviving owning project to an unfiltered run, so removing the final test class cannot schedule an empty filter. Test helpers default to the full owning suite; a helper may use a checked class or namespace map only when syntax-tree contracts prove every current consumer. Changes to a focused helper, its test project, or a syntax-proven consumer on either side of the edge revalidate that map. Project-wide test configuration and shared test infrastructure select full suites. Exact-head existence prevents deleted test projects from being scheduled and limits draw.io parsing to surviving authenticated blobs. Frontend and verifier changes receive their dedicated contracts; every path is classified and the exact diff is checked. Qualification collects no coverage, does not claim complete test inventory, and grants no merge authority. |
...
This split keeps feature development moving without pretending that an impact-selected signal is certification. The checked-in map fails closed on an unknown path, and its contract proves every currently tracked path has ownership. It conservatively selects all tests for unmapped helper sources, shared test/build infrastructure, runsettings, and cross-project linked fixtures, and never derives merge authority from the selected subset. A reviewed helper can name a smaller set only when the contract syntax-parses every tracked consumer and proves the exact consumer-class or consumer-namespace set still matches the checked map; the qualifier checks changed C# blobs from both merge-base and head, so adding, removing, or renaming a consumer reruns the contract before the checked map can become stale. Class filtering is limited to test-only C# files with a syntax-tree-proved direct xUnit method, exactly one filename-matching top-level test class, and project-owned namespace; Roslyn reads real declarations rather than test-shaped comment or string text, and any production owner on the same edge restores the full owning suite. A reviewed helper may also select exact consumer classes when every syntax-proven reference belongs directly to those classes; otherwise it retains all checked consumer namespaces. Test execution and diagram validation are bound to exact-head objects rather than stale checkout paths. Qualification uses two dependency-safe scheduler waves rather than a barrier after each concern. Independent build, frontend, and verifier-contract work shares the first four-unit wave; after build and frontend prerequisites are proved, selected tests, workflow validation, changed-file formatting, and exact diff-check share the second. Verifier contracts admit no more than two process-heavy processes at a time, and each suite uses its own GUID-scoped temporary root. Changed-file whitespace and IDE1006 checks share one `dotnet format` workspace load while preserving both diagnostics. A coherent draft may be published after qualification is green. Agents do not wait for or repeatedly trigger full coverage and browser certification after every patch. Marking the stable reviewed head/base pair ready starts merge-ref promotion; any later commit returns that head to qualification, requires promotion again, and cancels superseded Windows qualification, verifier, and browser work. Retargeting a pull request invalidates qualification, promotion, CodeQL, and dependency evidence because the comparison base changed; title- or body-only edits skip the substantive jobs under distinct non-required metadata context names, so they neither consume runners nor overwrite required evidence with a skipped conclusion. Deterministic failures block the slice. An infrastructure timeout gets one diagnostic rerun, then becomes verifier-health work instead of an unbounded feature-branch retry loop. Repeated cold timing samples belong to verifier-health changes and scheduled evidence, not every feature PR.

The verifier defaults to `Release`; required workflows pass `-Configuration Release` explicitly. `Debug` remains an explicit local opt-in through `-Configuration Debug`. The watchdog accepts the 300-second qualification deadline and the workflow's 900-second promotion deadline, but rejects any larger value. The selected deadline is inclusive; any later tick, cancellation, child timeout, nonzero exit, or missing/duplicate terminal marker fails closed. Tool setup and diagnostic upload remain outside the measured child and the workflow job ceiling. The stress workflow always runs its diagnostic-upload step, retains available TRX timing and failure diagnostics for 30 days, and fails if the expected artifacts are absent. Exact filters plus `TreatNoTestsAsError` make removal, renaming, or trait loss fail the job instead of silently producing an empty pass.
...
Qualification has an exact five-minute limit and owns the development-feedback contract. Promotion is an exhaustive once-per-stable-head certification pass with a bounded fifteen-minute limit. `scripts/verify-with-watchdog.ps1` measures either child with `System.Diagnostics.Stopwatch`, terminates its process tree at the selected deadline, captures combined output, labels the evidence as `qualification` or `promotion`, and accepts exactly one terminal record of the form `VERIFY_COMPLETE schema_version=1 status=passed elapsed_seconds=...`. Qualification retains its plan, logs, and selected TRX under `tests/QualificationResults`; promotion retains its complete phase, inventory, coverage, and watchdog evidence under `tests/VerificationResults`. A breach is a verifier incident to investigate, not authority to remove tests, reduce coverage, weaken assertions, or turn certification back into the draft edit loop.
