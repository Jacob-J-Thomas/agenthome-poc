...
The Pull Request tier starts with one bounded dependency-aware preflight DAG. The isolated Release build runs beside the independent ordered frontend install/lint/format/test chain and one explicitly classified descendant-heavy contract at a time. Descendant-heavy suites use distinct temporary process trees, immutable repository reads, and three logical units; ordinary source/temp-only bounded-phase, inventory, and watchdog contracts backfill only admitted capacity. Two process-heavy phases depend explicitly on the successful build: the coverage contract and a test-plan phase that copies and authenticates exact outputs, discovers all nine canonical assemblies sequentially, reconciles the stable-ID partition, and atomically publishes one schema-1 preparation plan. Scheduling priority therefore cannot admit either phase against stale or partial build output. The DAG uses no more than four tool processes, eight logical units, two process-heavy workers, and one CPU-bound worker. Every phase retains its original assertions, explicit weight, class limit, log, timeout, and fail-closed aggregate result, and no partial preflight boundary is claimed before the entire DAG passes.

Required gates first revalidate and hydrate the exact schema-1 preparation plan, including current ownership metadata, project and lane topology, filters, paths, run settings, environments, and immutable manifests; they do not repeat output copying, discovery, or partitioning. They then use twelve logical resource units but no more than four outer processes on the four-core hosted runner. Deterministic longest-processing-time-first ordering uses checked-in duration estimates. Persistence is split into four measured class shards. Startup uses three isolated process lanes for its six intentionally serialized runtime wrappers plus two balanced remainder lanes, with each wrapper still serialized inside its lane and the default-capability trust collection kept together. Integration and Web each use three isolated class shards; the other five test assemblies remain one lane each. The resulting twenty exact test lanes across nine canonical assemblies, the combined formatter, and `git diff --check` form twenty-two checked-in required-gate profiles. Every coverage-instrumented lane consumes three units, at most four run together, and each VSTest process retains the assembly's explicit two-thread xUnit ceiling. One combined CPU-bound C# format phase reserves six units and waits for enough logical capacity rather than oversubscribing a four-heavy wave. The phase omits a formatter subcommand intentionally: `dotnet format EmbodySense.sln --verify-no-changes --no-restore --severity warn --diagnostics IDE1006 --verbosity minimal` runs whitespace formatting and the explicitly selected IDE1006 style analyzer together. It joins the required schedule only after the authenticated plan proves build-output isolation, discovery, and exact partition reconciliation, allowing it to overlap immutable test execution without racing build artifacts. Missing profiles, unsafe or duplicate class prefixes, unknown or cyclic dependencies, oversized weights, underweighted resource classes, and limits above the effective worker ceiling fail before execution. Discovery and reconciliation remain hardware-bounded. Tests that share Startup runtime state, default capability-trust state, ephemeral API-host resources, or process-global state remain together or serialized through explicit collections; process-global cases disable collection parallelization entirely. Coverage child-process helper contracts take their immutable assembly directory as an explicit test coordinate, keeping those self-tests eligible for bounded execution without changing child-process isolation or coverage collection. Successful covered child startup has a 60-second readiness bound, fails immediately when either child exits, and captures bounded stdout/stderr evidence. Because both children must be ready before the parent releases the race, an early child waits for that complete sibling-readiness window plus a separately bounded 15-second release margin. Codex resolver and Web protocol fixtures use bounded compiled `CancellationHost` modes instead of generated shell-language servers, while preserving real operating-system process, standard-input, standard-output, timeout, and cancellation boundaries; the production resolver default remains exactly fifteen seconds and its test seam accepts only a shorter positive deadline. Every test assembly is still discovered once as a canonical selection. The partition reconciler uses xUnit's stable test-case unique ID to require exact, disjoint, nonempty lane coverage; execution reconciliation then requires every VSTest test-case ID in the expected inventory and rejects missing, unexpected, failed, or cross-report duplicate executions. Dynamic data may produce multiple execution rows for one discovered case only within its admitted lane report.
...
Exact-head Windows run 31753376066 executed all 5,126 canonical tests as 5,240 passing dynamic rows across all nine lanes and emitted their coverage reports. Final threshold and inventory reconciliation began at 598.289 seconds and the unchanged watchdog terminated it. Exact-head run 31757214113 then proved that the physical second formatter load was still material: naming and whitespace consumed 90.979 and 46.430 seconds. That run also exposed a Windows-contention defect in the successful covered admission-writer race: one child had not published readiness inside the prior 15-second startup bound, so Persistence completed 1,662 of 1,663 tests successfully and failed before Web, Integration, inventory reconciliation, or coverage thresholds could complete. An isolated .NET 10 fixture proved the combined command fails independently for a whitespace-only defect and an IDE1006-only defect. Two alternating full-solution local measurements recorded sequential whitespace-plus-style totals of 49.65 and 34.86 seconds versus combined totals of 37.51 and 34.77 seconds. The warm comparison is effectively equal, while the combined command removes one physical workspace load and retains both logical checks. Local controlled covered comparisons proved exact production coverable/hit-set parity for the Application maximum-history fixture and the Startup process split; repeated Web coverage produced the same hit set after accounting for a nondeterministic cancellation branch, and Integration's incidental catalog-lock contention lines remain deterministically covered by the exact Persistence fail-closed lock regression. The current isolated-shard profile has a conservative 455-second required-gate-only virtual bound; neither that model nor local shard timing substitutes for exact-head Windows whole-run timing, coverage, and inventory proof.
...
Exact-head acceptance requires repeated Windows runs at or below the five-minute target, with cold/warm variance recorded from retained manifests and per-phase logs. The hard contract remains 600 seconds even if a transient runner misses the target. Any over-limit run fails and must be escalated with its critical path before this baseline is revised. Exact-head run 31671865209 admitted all four whole internally parallel assemblies together and reached the watchdog at 600.089 seconds. Three-heavy exact-head runs 31749217561 and 31751941059 then reached the watchdog at 600.055 and 600.204 seconds respectively; in both, the Persistence lane remained active without a terminal TRX while the other eight assembly lanes completed. The current design replaces those whole-assembly overlaps with exact immutable class shards and one dependency-aware preflight DAG. Its conservative required-gate profile has a 455-second virtual makespan, excluding preflight, and remains pending authoritative exact-head Windows whole-run timing, exact test inventory, and coverage proof.
