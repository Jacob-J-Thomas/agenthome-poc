...
public static class CapabilityLifecycleFactory
{
    ...
    {
        return Compose(paths, catalogTrustProvider, artifactStateTrustProvider, artifactTrustVerifier, auditLog, roleSource, scheduleSource).Lifecycle;
    }

    /// <summary>Creates the browser-safe selection orchestrator without exposing client-owned descriptors or digests.</summary>
    /// <param name="paths">The initialized workspace paths.</param>
    /// <param name="catalogTrustProvider">The server-owned catalog and lifecycle trust provider.</param>
    /// <param name="artifactStateTrustProvider">The server-owned immutable artifact trust provider.</param>
    /// <param name="artifactTrustVerifier">The server-owned artifact policy verifier.</param>
    ...
    /// <param name="roleSource">The optional explicitly registered future role-domain adapter.</param>
    /// <param name="scheduleSource">The optional explicitly registered future schedule-domain adapter.</param>
    /// <returns>The fully composed browser-safe lifecycle selection service.</returns>
    public static CapabilityLifecycleSelectionService CreateSelection(
        WorkspacePaths paths,
        ICapabilityCatalogTrustProvider catalogTrustProvider,
        ICapabilityArtifactStateTrustProvider artifactStateTrustProvider,
        ICapabilityArtifactTrustVerifier artifactTrustVerifier,
        ...
        IScheduleCapabilityDependentIndexSource? scheduleSource = null)
    {
        var composition = Compose(paths, catalogTrustProvider, artifactStateTrustProvider, artifactTrustVerifier, auditLog, roleSource, scheduleSource);
        return new CapabilityLifecycleSelectionService(composition.TargetResolver, composition.Lifecycle);
    }

    private static (CapabilityLifecycleService Lifecycle, ICapabilityLifecycleTargetResolver TargetResolver) Compose(
        WorkspacePaths paths,
        ICapabilityCatalogTrustProvider catalogTrustProvider,
        ICapabilityArtifactStateTrustProvider artifactStateTrustProvider,
        ICapabilityArtifactTrustVerifier artifactTrustVerifier,
        IAuditLog auditLog,
        IRoleCapabilityDependentIndexSource? roleSource,
        IScheduleCapabilityDependentIndexSource? scheduleSource)
    {
        ...
        return (new CapabilityLifecycleService(index, baseline, artifacts, lifecycle, auditLog, authorityTransaction), artifacts);
    }
}
