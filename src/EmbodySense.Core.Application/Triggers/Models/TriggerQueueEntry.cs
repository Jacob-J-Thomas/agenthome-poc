...
/// <param name="WorkerLease">The current or last generation-scoped worker ownership evidence.</param>
/// <param name="Dispatch">The durable dispatch intent and outcome evidence.</param>
/// <param name="WorkspaceId">The exact trusted workspace identity retained by the trigger envelope.</param>
/// <param name="TargetGraphId">The exact governed graph identity, or null for a fenced legacy target.</param>
/// <param name="TargetRevisionId">The exact governed revision identity, or null for a fenced legacy target.</param>
public sealed record TriggerQueueEntry(TriggerDeliveryId DeliveryId, TriggerDeduplicationId DeduplicationId, string LoopId, string CanonicalEnvelopeHash, int SerializedEntryBytes, int QueuedReservationBytes, int RetainedReservationBytes, TriggerQueueEntryState State, TriggerQueueTerminalReason TerminalReason, TriggerQueueOrderKey OrderKey, long Revision, DateTimeOffset RecordedAtUtc, DateTimeOffset? TerminalAtUtc, TriggerAdmissionStatus AdmissionStatus, TriggerAdmissionReason AdmissionReason, TriggerWorkerLease? WorkerLease = null, TriggerDispatchEvidence? Dispatch = null, string? WorkspaceId = null, string? TargetGraphId = null, string? TargetRevisionId = null);
