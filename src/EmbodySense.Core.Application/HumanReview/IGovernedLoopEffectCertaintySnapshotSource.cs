...
{
    /// <summary>Reads the current exact certainty for one immutable workspace-scoped effect-attempt identity and preparation expectation.</summary>
    /// <remarks>Implementations must re-read canonical durable truth in the queried workspace and return a detached result. They must never call a transition or lease-acquisition API to serve this query, echo untrusted fields as proof, or expose raw effect payloads.</remarks>
    /// <param name="query">The exact canonical workspace-scoped identity and value-free preparation expected by the Human Review continuation.</param>
