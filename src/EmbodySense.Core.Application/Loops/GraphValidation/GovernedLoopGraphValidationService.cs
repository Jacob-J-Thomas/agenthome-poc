...
public sealed class GovernedLoopGraphValidationService
{
    ...
    private static void ValidateCatalogDescriptor(GovernedLoopNodeCatalogDescriptor descriptor, List<GovernedLoopGraphValidationError> errors)
    {
        ...
        if (GovernedLoopTopologyNodeCatalogContract.TryResolve(descriptor.Descriptor, out _)
            && !GovernedLoopTopologyNodeCatalogContract.HasExactCatalogSemantics(descriptor))
        {
            Add(
                errors,
                "catalog.topology-contract.mismatch",
        ...
        List<GovernedLoopGraphValidationError> errors)
    {
        if (!GovernedLoopTopologyNodeCatalogContract.TryResolve(node.Descriptor, out var canonical)
            || canonical is null
            || !GovernedLoopTopologyNodeCatalogContract.HasExactCatalogSemantics(admittedDescriptor))
        {
            return;
        }
    ...
    private static void ValidateJoins(GovernedLoopGraphDefinition graph, IReadOnlyDictionary<string, GovernedLoopNodeCatalogDescriptor> semantics, List<GovernedLoopGraphValidationError> errors)
    {
        foreach (var node in graph.Nodes.OrderBy(node => node.Id, StringComparer.Ordinal))
        {
            ...
            if (descriptor.JoinPolicy == GovernedLoopJoinPolicy.All
                && !GovernedLoopControlTopologySemantics.AreAllJoinInputsJointlySatisfiable(graph, node.Id, incoming))
            {
    ...
    private static void ValidateCycles(GovernedLoopGraphDefinition graph, IReadOnlyDictionary<string, GovernedLoopNodeCatalogDescriptor> semantics, List<GovernedLoopGraphValidationError> errors)
    {
        ...
        foreach (var component in StronglyConnectedComponents(graph.Nodes.Select(node => node.Id), adjacency))
        {
            ...
            if (!cyclic)
            {
                ...
                if (!string.Equals(nodeId, graph.EntryNodeId, StringComparison.Ordinal)
                    && node.Descriptor.Kind != GovernedLoopNodeKind.Join
                    && graph.ControlEdges.Count(edge => string.Equals(edge.ToNodeId, nodeId, StringComparison.Ordinal)) != 1)
                {
                    Add(
                        errors,
                        "node.control-arrival.ambiguous",
                        GovernedLoopGraphElementKind.Node,
                        nodeId,
                        $"graph.nodes[{nodeId}]",
                        "An acyclic non-entry node without explicit Join semantics must have exactly one incoming control edge.");
                }
            ...
            foreach (var edge in graph.ControlEdges
                         .Where(edge => componentNodeIds.Contains(edge.FromNodeId) && !componentNodeIds.Contains(edge.ToNodeId))
                         .OrderBy(edge => edge.Id, StringComparer.Ordinal))
            {
                var internalEdges = graph.ControlEdges
                    .Where(candidate => string.Equals(candidate.FromNodeId, edge.FromNodeId, StringComparison.Ordinal)
                        && componentNodeIds.Contains(candidate.ToNodeId))
                    .ToArray();
                    ...
                    && !GovernedLoopControlTopologySemantics.AreMutuallyExclusive(internalEdges[0].Condition, edge.Condition))
                {
                    Add(
                        errors,
                        "edge.cycle-exit.not-exclusive",
                        GovernedLoopGraphElementKind.ControlEdge,
                        edge.Id,
                        $"graph.controlEdges[{edge.Id}]",
                        "A cycle exit must be mutually exclusive with the source node's loop-continuation outcome.");
                }
            }
