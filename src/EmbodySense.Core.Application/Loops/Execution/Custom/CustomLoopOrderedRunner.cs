...
public sealed class CustomLoopOrderedRunner : ICustomLoopResumeExecutor, ICustomLoopExecutionCancellationSignal
{
    ...
    internal async Task<CustomLoopOrderedRunResult> ResumeWaitSequentialAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var context = CreateSequentialContext(
            ...
            auditRecorder);
        if (context is null
            || request.ActivationOrdinal < 0
            || !IsHash(request.ContinuationEvidenceHash)
            || string.IsNullOrWhiteSpace(request.Actor))
        {
            return Result(CustomLoopOrderedRunStatus.InvalidState, null, "The canonical Wait resume hand-off is invalid and no ordered runtime work was dispatched.");
        }
        ...
        {
            return Result(CustomLoopOrderedRunStatus.Failed, null, $"The resumed Wait run trace could not be loaded safely: {SafeExceptionClass(exception)}.");
        }
        ...
        if (!SequentialRunMatches(run, context)
            ...
            || run.Frontier?.Payload.Nodes.ElementAtOrDefault(request.ActivationOrdinal) is not { } activation
            || activation.Descriptor.Kind != GovernedLoopNodeKind.Wait
            || run.WaitEvidence.SingleOrDefault(item => item.ActivationOrdinal == request.ActivationOrdinal) is not { ContinuationEvidence: { } continuation }
            || !string.Equals(continuation.ContentHash, request.ContinuationEvidenceHash, StringComparison.Ordinal))
        {
            return Result(CustomLoopOrderedRunStatus.InvalidState, run, "The durable Wait continuation no longer matches the exact immutable graph, activation, and evidence chain.");
        }

        if (activation.Status == GovernedLoopNodeExecutionStatus.Completed)
        {
            var node = context.Plan.Nodes.SingleOrDefault(candidate =>
                candidate.Ordinal == activation.PlanOrdinal
                && string.Equals(candidate.NodeId, activation.NodeId, StringComparison.Ordinal));
            var terminal = node is null || activation.Attempt is not { } completedAttempt
                ? null
                : FindSequentialNodeEvidence(run, node, activation, completedAttempt);
            if (terminal is null
                || !string.Equals(terminal.WaitContinuationEvidenceHash, continuation.ContentHash, StringComparison.Ordinal))
            {
                return Result(CustomLoopOrderedRunStatus.InvalidState, run, "The durable Wait completion has no exact authenticated terminal evidence for audit reconciliation.");
            }

            var outcomeAuditFailure = await AppendOutcomeAuditAsync(
                run,
                terminal,
                CreateWaitNodeAudit(run, request.Actor, terminal, AuditSchema.Outcomes.Succeeded),
                context.AuditRecorder,
                IntegrityToken()).ConfigureAwait(false);
            if (outcomeAuditFailure is not null)
            {
                var invalid = await TerminateAsync(
                    run,
                    request.Actor,
                    CustomLoopRunStatus.NeedsReview,
                    outcomeAuditFailure.FailureCode,
                    outcomeAuditFailure.Detail,
                    terminalAuditRecorder: context.AuditRecorder).ConfigureAwait(false);
                return invalid.Status == CustomLoopOrderedRunStatus.NeedsReview
                    ? invalid
                    : Result(CustomLoopOrderedRunStatus.NeedsReview, invalid.Run ?? run, outcomeAuditFailure.Detail);
            }

            return run.Status switch
            {
                CustomLoopRunStatus.Waiting => Result(CustomLoopOrderedRunStatus.Waiting, run, "The exact Wait completion is durable and another activation remains parked."),
                CustomLoopRunStatus.Completed => await CompleteCanonicalAsync(run, context, "The exact durable canonical completion was replayed after Wait continuation.", run.FinalOutput ?? string.Empty),
                CustomLoopRunStatus.Running => await ContinueOwnedWaitAsync(run, request.Actor, context, cancellationToken).ConfigureAwait(false),
                _ => Result(CustomLoopOrderedRunStatus.InvalidState, run, "The exact Wait completion is durable under an incompatible aggregate lifecycle state."),
            };
        }
        ...
        CancellationToken cancellationToken)
    {
        using var ownership = TryRegisterActiveRun(run.Id);
        return ownership is null
            ? Result(CustomLoopOrderedRunStatus.Failed, run, "This runtime is already coordinating ordered execution for the resumed Wait run.")
            : await ContinueRegisteredAsync(run, actor, cancellationToken, context).ConfigureAwait(false);
    }
        ...
        string? sequentialCapabilityFailure)
    {
        while (true)
        {
            ...
            if (selected.Status == GovernedLoopSequentialFrontierSelectionStatus.Waiting)
            {
                return Result(CustomLoopOrderedRunStatus.Waiting, run, selected.Detail);
            }
                ...
                && CycleClaimDeadlineFailure(run, context.Plan, node, selected.Activation!, Now(run)) is { } cycleDeadlineFailure)
            {
                return await TerminateAsync(
                    ...
                    actor,
                    CustomLoopRunStatus.NeedsReview,
                    "canonical_cycle_duration_exceeded",
                    cycleDeadlineFailure);
            }
            ...
            {
                if (GovernedLoopSequentialNodeDescriptors.IsWait(node.Descriptor))
                {
                    var resumedWait = await DispatchSelectedSequentialNodeAsync(
                        context,
                        run,
                        node,
                        selected.Attempt!.Value,
                        ...
                        dispatchState,
                        cancellationToken);
                    if (resumedWait.Terminal is not null)
                    {
                        return resumedWait.Terminal;
                    }
            ...
            if (deadlineReached)
            {
                ...
                var deadlineClaim = isPure
                    ...
                    : GovernedLoopSequentialNodeDescriptors.IsWait(node.Descriptor)
                        ? await ClaimSequentialWaitNodeAsync(run, context, node, deadlineOperationId, actor, cancellationToken)
                ...
                var isWait = GovernedLoopSequentialNodeDescriptors.IsWait(node.Descriptor);
                var rejected = await DispatchSequentialNodeAsync(
                    context,
                    deadlineClaim.Run!,
                    node,
                    ...
                    token => isPure
                        ? RejectSequentialPureNodeAsync(
                            deadlineClaim.Run!,
                            actor,
                            new SequentialNodeExecutionContext(context.Anchor.AdapterBinding, context.Artifact, context.Plan, node, RequireRunningSequentialActivation(deadlineClaim.Run!, node, 1, deadlineOperationId), 1, deadlineOperationId, context.AllowedCapabilityIds, context.AuditRecorder),
                            "run_deadline_exceeded",
                            CanonicalDeadlineRejectionDetail)
            ...
            var claimed = GovernedLoopSequentialNodeDescriptors.IsPure(node.Descriptor)
                ...
                : GovernedLoopSequentialNodeDescriptors.IsWait(node.Descriptor)
                    ? await ClaimSequentialWaitNodeAsync(run, context, node, operationId, actor, cancellationToken)
    ...
    private async Task<RunAdvance> ClaimSequentialWaitNodeAsync(
        CustomLoopRunRecord run,
        SequentialExecutionContext context,
        GovernedLoopSequentialPlanNode node,
        string attemptOperationId,
        ...
        CancellationToken cancellationToken)
    {
        if (!GovernedLoopSequentialNodeDescriptors.IsWait(node.Descriptor))
        {
            return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.InvalidState, run, "Only an admitted Wait may use the atomic Wait claim."));
        }
        ...
        var activation = transition.Frontier.Payload.Nodes[selection.Activation!.ActivationOrdinal];
        var sequentialNode = new SequentialNodeExecutionContext(
            context.Anchor.AdapterBinding,
            context.Artifact,
            context.Plan,
        ...
        var start = Event(
            ...
            now,
            CustomLoopRunEventKind.NodeAttemptStarted,
            "Wait dispatch was retained before the activation released ordered execution ownership.",
        ...
        start = WithSequentialEvidence(start, sequentialNode, CustomLoopSequentialNodeEvidenceKind.DispatchStarted, CustomLoopSequentialNodeDisposition.Unknown);
        var candidate = Append(run, now, [start]) with { Frontier = transition.Frontier };
        var capacityBoundary = await RejectUnavailableTraceCapacityAsync(run, candidate, actor, "wait-node", cancellationToken);
        if (capacityBoundary is not null)
        {
            return capacityBoundary;
        }
        ...
        {
            var latest = await _runStore.GetAsync(run.Id, IntegrityToken()).ConfigureAwait(false);
            var cancelled = await CancelBeforeDispatchAsync(latest ?? run, actor, context.AuditRecorder).ConfigureAwait(false);
            return new RunAdvance(cancelled.Run, cancelled);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var ambiguityDetail = terminalEvent.Detail;
        CustomLoopRunEvent? lifecycleEvent = null;
        if (!isCompleted)
        {
            var terminalOwner = run with { Events = [.. run.Events, startEvent, terminalEvent] };
            lifecycleEvent = Event(terminalOwner, now, CustomLoopRunEventKind.LifecycleChanged, ambiguityDetail);
        }

        var candidate = Append(
            run,
            now,
            lifecycleEvent is null ? [startEvent, terminalEvent] : [startEvent, terminalEvent, lifecycleEvent]) with
        {
            Status = isCompleted ? run.Status : CustomLoopRunStatus.NeedsReview,
            CompletedAtUtc = isCompleted ? run.CompletedAtUtc : now,
            ExecutionClock = isCompleted ? run.ExecutionClock : AdvanceClock(run.ExecutionClock, now, terminal: true),
            FailureCode = isCompleted ? run.FailureCode : "canonical_condition_route_ambiguous",
            FailureDetail = isCompleted ? run.FailureDetail : ambiguityDetail,
            Frontier = frontier,
        };
        ...
        var durableActivation = durable.Frontier!.Payload.Nodes[activation.ActivationOrdinal];
        var startAuditFailure = await ReconcileTopologyNodeStartAuditAsync(
            durable,
            node,
            durableActivation,
            1,
            ...
            actor,
            context.AuditRecorder);
        var durableTerminal = durable.Events.Single(item => string.Equals(item.EventId, terminalEvent.EventId, StringComparison.Ordinal));
        var outcomeAuditFailure = startAuditFailure is null
            ? await AppendOutcomeAuditAsync(
                durable,
                durableTerminal,
                CreateTopologyNodeAudit(durable, actor, durableTerminal, AuditSchema.Outcomes.NeedsReview),
                context.AuditRecorder,
                IntegrityToken())
            : null;
        if (startAuditFailure is not null || outcomeAuditFailure is not null)
        {
            var failure = startAuditFailure ?? outcomeAuditFailure!;
            return new RunAdvance(null, await AppendTerminalIntegrityWarningAsync(durable, CustomLoopOrderedRunStatus.NeedsReview, $"{ambiguityDetail} {failure.Detail}"));
        }

        var audited = await CompleteSequentialPureTerminalLifecycleAuditAsync(
            durable,
            CustomLoopRunStatus.NeedsReview,
            "canonical_condition_route_ambiguous",
            ambiguityDetail,
            lifecycleEvent!,
            context.AuditRecorder);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (cancellationToken.IsCancellationRequested
            ...
            && run.Frontier?.Payload.Nodes.SingleOrDefault(candidate =>
                ...
                && string.Equals(candidate.AttemptOperationId, attemptOperationId, StringComparison.Ordinal)) is not null)
        {
            return GovernedLoopSequentialNodeDescriptors.IsWorkspaceAction(node.Descriptor)
                ? await DispatchAndAdvanceSequentialWorkspaceActionAsync(
                    context,
                    run,
                    node,
                    attempt,
                    ...
                    actor,
                    IntegrityToken())
                : await DispatchAndAdvanceSequentialCommandActionAsync(
                    context,
                    run,
                    node,
                    attempt,
        ...
        if (GovernedLoopSequentialNodeDescriptors.IsCommandAction(node.Descriptor))
        {
            return await DispatchAndAdvanceSequentialCommandActionAsync(
                context,
                run,
                node,
                attempt,
        ...
        if (GovernedLoopSequentialNodeDescriptors.IsWait(node.Descriptor))
        {
            return await DispatchAndAdvanceSequentialWaitNodeAsync(
                context,
                run,
                node,
                attempt,
    ...
    private async Task<RunAdvance> DispatchAndAdvanceSequentialCommandActionAsync(
        SequentialExecutionContext context,
        CustomLoopRunRecord run,
        GovernedLoopSequentialPlanNode node,
        int attempt,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var prepared = await DispatchSequentialNodeAsync(
            ...
            attempt,
            actor,
            token => PrepareOrExecuteSequentialCommandActionAsync(context, run, node, attempt, attemptOperationId, actor, token),
            cancellationToken);
        ...
        if (frontier is null)
        {
            var terminal = await TerminateAsync(prepared.Run!, actor, CustomLoopRunStatus.NeedsReview, "canonical_frontier_advancement_failed", "Canonical command Action evidence could not advance the exact Running frontier.");
            return new RunAdvance(terminal.Run, terminal);
        }
        return await CommitCheckpointAsync(
            prepared.Run!,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var sequentialNode = new SequentialNodeExecutionContext(
            ...
            context.AllowedCapabilityIds,
            context.AuditRecorder);
        var terminal = FindSequentialNodeEvidence(run, node, activation, attempt);
        if (terminal is not null)
        {
            return await ReconcileSequentialCommandActionOutcomeAsync(run, actor, sequentialNode, terminal);
        }
        if (!CommandActionNodeDescriptors.IsCommandAction(node.Descriptor)
            || node.Parameters.Count > CommandActionContractLimits.MaxSlots)
        {
            return await RejectSequentialNodeBeforeProviderAsync(
                ...
                node.NodeId,
                isExit: false,
                "command_action_input_invalid",
                "The exact immutable structured command Action input is invalid.");
        }
        if (_commandActionExecutor is null)
        {
            return await RejectSequentialNodeBeforeProviderAsync(
                run,
                actor,
                ...
                node.NodeId,
                isExit: false,
                "command_action_executor_unavailable",
                "No canonical structured command Action executor is composed.");
        }
        ...
        if (started is null)
        {
            var start = Event(
                ...
                Now(run),
                CustomLoopRunEventKind.NodeAttemptStarted,
                "Canonical structured command Action dispatch was retained before effect orchestration.",
                activation.CycleIteration ?? run.Checkpoint.Iteration,
                node.NodeId,
                attempt,
                traceReservationUtf8Bytes: CustomLoopLimits.MaxGraphPureNodeOutcomeEvidenceReservationUtf8Bytes,
                eventId: attemptOperationId);
            start = WithSequentialEvidence(start, sequentialNode, CustomLoopSequentialNodeEvidenceKind.DispatchStarted, CustomLoopSequentialNodeDisposition.Unknown);
            var persistedStart = await PersistSequentialRecoverableActionEventAsync(run, node, activation, attempt, start, outcomeMayExist: false);
            if (persistedStart.Terminal is not null)
            {
        ...
        try
        {
            result = await _commandActionExecutor.ExecuteAsync(
                new GovernedLoopCommandActionExecutionRequest(
        ...
        catch (OperationCanceledException) when (actionToken.IsCancellationRequested)
        {
            result = new GovernedLoopCommandActionExecutionResult(
                ...
                null,
                "Command Action cancellation raced its canonical effect protocol; automatic relaunch is forbidden.");
        }
        catch (Exception exception)
        {
            result = new GovernedLoopCommandActionExecutionResult(
                GovernedLoopCommandActionExecutionStatus.NeedsReview,
                null,
                $"The canonical command Action executor failed closed ({SafeExceptionClass(exception)}).");
        }

        var parsedResult = result.CanonicalOutput is { } canonicalOutput
            ...
            && result.CanonicalOutput is { } completedOutput)
        {
            var completed = Event(
                ...
                "Canonical structured command Action outcome was committed.",
                activation.CycleIteration ?? run.Checkpoint.Iteration,
                node.NodeId,
                attempt,
                output: completedOutput,
                originalOutputCharacters: completedOutput.Length,
                truncated: false,
                retained: true,
                published: false);
            ...
            var persisted = await PersistSequentialRecoverableActionEventAsync(run, node, activation, attempt, completed, outcomeMayExist: true);
            return persisted.Terminal is not null
                ? persisted
                : await ReconcileSequentialCommandActionOutcomeAsync(persisted.Run!, actor, sequentialNode, FindSequentialNodeEvidence(persisted.Run!, node, activation, attempt)!);
        }

        var conclusiveFailure = result.Status == GovernedLoopCommandActionExecutionStatus.Failed
            && parsedResult?.Outcome == CommandActionResultOutcome.Failed;
        var needsReview = !conclusiveFailure && result.Status != GovernedLoopCommandActionExecutionStatus.Rejected;
        var failure = Event(
            run,
            Now(run),
            CustomLoopRunEventKind.NodeAttemptFailed,
            ...
            node.NodeId,
            attempt,
            output: conclusiveFailure ? result.CanonicalOutput : null,
            originalOutputCharacters: conclusiveFailure ? result.CanonicalOutput!.Length : null,
        ...
        var retainedFailure = await PersistSequentialRecoverableActionEventAsync(run, node, activation, attempt, failure, outcomeMayExist: conclusiveFailure || needsReview);
        return retainedFailure.Terminal is not null
            ? retainedFailure
            : await ReconcileSequentialCommandActionOutcomeAsync(retainedFailure.Run!, actor, sequentialNode, FindSequentialNodeEvidence(retainedFailure.Run!, node, activation, attempt)!);
    }

    private async Task<RunAdvance> ReconcileSequentialCommandActionOutcomeAsync(
        CustomLoopRunRecord run,
        string actor,
        SequentialNodeExecutionContext sequentialNode,
        CustomLoopRunEvent terminal)
    {
        if (terminal.SequentialNodeEvidence is
            {
                Kind: CustomLoopSequentialNodeEvidenceKind.CompletedOutcome,
                Disposition: CustomLoopSequentialNodeDisposition.Completed,
            }
            && terminal.CanonicalOutput is { } output
            && CommandActionResultContract.TryParse(output, out var result)
            && result!.Outcome == CommandActionResultOutcome.Succeeded)
        {
            var auditFailure = await AppendOutcomeAuditAsync(
                run,
                terminal,
                CreateCommandActionAudit(run, actor, terminal, AuditSchema.Outcomes.Succeeded),
            ...
            var retained = new CustomLoopRetainedOutput(
                ...
                output,
                CustomLoopTraceContentHash.Compute(output));
            return new RunAdvance(run, null, run.Checkpoint with { CurrentIterationResult = retained });
        }
            ...
            })
        {
            var auditFailure = await AppendOutcomeAuditAsync(
                run,
                terminal,
                CreateCommandActionAudit(run, actor, terminal, AuditSchema.Outcomes.Failed),
                sequentialNode.AuditRecorder,
                IntegrityToken());
            if (auditFailure is not null)
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var prepared = await DispatchSequentialNodeAsync(
            ...
            attempt,
            actor,
            token => PrepareOrExecuteSequentialWorkspaceActionAsync(context, run, node, attempt, attemptOperationId, actor, token),
            cancellationToken);
        if (prepared.Terminal is not null)
        {
            return prepared;
        ...
        if (frontier is null)
        {
            var terminal = await TerminateAsync(prepared.Run!, actor, CustomLoopRunStatus.NeedsReview, "canonical_frontier_advancement_failed", "Canonical workspace Action evidence could not advance the exact Running frontier.");
            return new RunAdvance(terminal.Run, terminal);
        }
        ...
        return await CommitCheckpointAsync(
            prepared.Run!,
            prepared.PendingCheckpoint,
            $"Workspace Action checkpoint committed after `{node.NodeId}`.",
            frontier.Frontier,
            frontier.SkipEvents);
    }

    private async Task<RunAdvance> PrepareOrExecuteSequentialWorkspaceActionAsync(
        SequentialExecutionContext context,
        CustomLoopRunRecord run,
        GovernedLoopSequentialPlanNode node,
        int attempt,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (terminal is not null)
        {
            return await ReconcileSequentialWorkspaceActionOutcomeAsync(run, actor, sequentialNode, terminal);
        }

        if (!node.Parameters.TryGetValue("input", out var inputJson)
            || !WorkspaceActionNodeDescriptors.TryResolve(node.Descriptor, out var actionKind)
            || !WorkspaceActionInputContract.TryParse(inputJson, actionKind, out var input, out _)
            || !string.Equals(WorkspaceActionInputContract.Encode(input!), inputJson, StringComparison.Ordinal))
        {
            return await RejectSequentialNodeBeforeProviderAsync(
                run,
                actor,
                ...
                "The exact immutable workspace Action input is invalid.");
        }

        if (_workspaceActionExecutor is null)
        {
            return await RejectSequentialNodeBeforeProviderAsync(
                run,
                actor,
                ...
                node.NodeId,
                isExit: false,
                "workspace_action_executor_unavailable",
                "No canonical workspace Action executor is composed.");
        }
        ...
        try
        {
            result = await _workspaceActionExecutor.ExecuteAsync(
                new GovernedLoopWorkspaceActionExecutionRequest(
                    new GovernedLoopSequentialNodeDispatchRequest(
                        GovernedLoopSequentialNodeDispatchRequest.CurrentSchemaVersion,
                        context.Anchor,
                        context.Plan,
        ...
        catch (OperationCanceledException) when (actionToken.IsCancellationRequested)
        {
            result = new GovernedLoopWorkspaceActionExecutionResult(
                ...
                null,
                "Workspace Action cancellation raced its canonical effect protocol; automatic redispatch is forbidden.");
        }
        catch (Exception exception)
        {
            result = new GovernedLoopWorkspaceActionExecutionResult(
            ...
            && WorkspaceActionResultContract.TryParse(output, out _))
        {
            var completed = Event(
                ...
                Now(run),
                CustomLoopRunEventKind.NodeAttemptCompleted,
                "Canonical workspace Action outcome was committed.",
                activation.CycleIteration ?? run.Checkpoint.Iteration,
                node.NodeId,
                attempt,
                output: output,
                originalOutputCharacters: output.Length,
                truncated: false,
                retained: true,
                published: false);
            completed = WithSequentialEvidence(completed, sequentialNode, CustomLoopSequentialNodeEvidenceKind.CompletedOutcome, CustomLoopSequentialNodeDisposition.Completed);
            ...
            return persisted.Terminal is not null
                ? persisted
                : await ReconcileSequentialWorkspaceActionOutcomeAsync(persisted.Run!, actor, sequentialNode, FindSequentialNodeEvidence(persisted.Run!, node, activation, attempt)!);
        }

        var needsReview = result.Status != GovernedLoopWorkspaceActionExecutionStatus.Rejected;
        var failure = Event(
            run,
            Now(run),
            CustomLoopRunEventKind.NodeAttemptFailed,
        ...
        failure = WithSequentialEvidence(
            ...
            needsReview ? CustomLoopSequentialNodeEvidenceKind.AmbiguityAttention : CustomLoopSequentialNodeEvidenceKind.DefinitiveRejection,
            needsReview ? CustomLoopSequentialNodeDisposition.NeedsReview : CustomLoopSequentialNodeDisposition.Rejected);
        var retainedFailure = await PersistSequentialRecoverableActionEventAsync(run, node, activation, attempt, failure, outcomeMayExist: needsReview);
        return retainedFailure.Terminal is not null
            ? retainedFailure
            : await ReconcileSequentialWorkspaceActionOutcomeAsync(retainedFailure.Run!, actor, sequentialNode, FindSequentialNodeEvidence(retainedFailure.Run!, node, activation, attempt)!);
    }

    private async Task<RunAdvance> ReconcileSequentialWorkspaceActionOutcomeAsync(
        CustomLoopRunRecord run,
        string actor,
        SequentialNodeExecutionContext sequentialNode,
        CustomLoopRunEvent terminal)
    {
            ...
            && WorkspaceActionResultContract.TryParse(output, out _))
        {
            var auditFailure = await AppendOutcomeAuditAsync(
                run,
                terminal,
                CreateWorkspaceActionAudit(run, actor, terminal, AuditSchema.Outcomes.Succeeded),
                sequentialNode.AuditRecorder,
                IntegrityToken());
            if (auditFailure is not null)
            {
            ...
            })
        {
            var auditFailure = await AppendOutcomeAuditAsync(
                run,
                terminal,
                CreateWorkspaceActionAudit(run, actor, terminal, AuditSchema.Outcomes.Failed),
                sequentialNode.AuditRecorder,
                IntegrityToken());
            if (auditFailure is not null)
            {
    ...
    private async Task<RunAdvance> PersistSequentialRecoverableActionEventAsync(
        ...
        bool outcomeMayExist)
    {
        var candidate = Append(run, runEvent.TimestampUtc, [runEvent]);
        try
        {
            var stored = await _runStore.UpdateAsync(candidate, run.LifecycleVersion, IntegrityToken());
            if (stored.Status == CustomLoopRunStoreStatus.Updated && stored.Run is not null)
            {
                return new RunAdvance(stored.Run, null);
            }
            if (stored.Status == CustomLoopRunStoreStatus.NotFound)
            {
                return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.NotFound, null, "The recoverable Action run disappeared during evidence persistence."));
            }
        }
        catch (Exception exception) when (exception is not UnsupportedCustomLoopRunDiscoveryIndexSchemaException)
        {
            // Read back the exact append below. A response-loss exception must not turn a durable
            // Action outcome into an ambiguous run or cause the effect protocol to redispatch it.
        }
        ...
        catch (Exception exception)
        {
            return outcomeMayExist
                ? await EscalatePostOutcomePersistenceUncertaintyAsync(run, $"The recoverable Action may have committed, but its ordered evidence readback failed ({SafeExceptionClass(exception)}).")
                : new RunAdvance(null, Result(CustomLoopOrderedRunStatus.Failed, run, $"The recoverable Action pre-dispatch evidence readback failed ({SafeExceptionClass(exception)})."));
        }

        var exact = latest is null
            ? null
            : runEvent.SequentialNodeEvidence?.Kind == CustomLoopSequentialNodeEvidenceKind.DispatchStarted
                ? FindSequentialDispatchStart(latest, node, activation, attempt, runEvent.EventId)
                : FindSequentialNodeEvidence(latest, node, activation, attempt);
        if (exact is not null
            && string.Equals(exact.EventId, runEvent.EventId, StringComparison.Ordinal)
            && string.Equals(exact.SequentialNodeEvidence?.EvidenceHash, runEvent.SequentialNodeEvidence?.EvidenceHash, StringComparison.Ordinal)
            && string.Equals(exact.CanonicalOutput, runEvent.CanonicalOutput, StringComparison.Ordinal))
        {
            return new RunAdvance(latest, null);
        }

        return outcomeMayExist
            ? await EscalatePostOutcomePersistenceUncertaintyAsync(run, "The recoverable Action may have committed, but its exact ordered outcome evidence could not be authenticated.")
            : new RunAdvance(null, Result(CustomLoopOrderedRunStatus.Conflict, latest, "The recoverable Action pre-dispatch evidence conflicted; no effect was started."));
    }

    private async Task<RunAdvance> DispatchAndAdvanceSequentialWaitNodeAsync(
        SequentialExecutionContext context,
        CustomLoopRunRecord run,
        GovernedLoopSequentialPlanNode node,
        int attempt,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var startAuditFailure = await ReconcileWaitNodeStartAuditAsync(
            ...
            attemptOperationId,
            actor,
            context.AuditRecorder).ConfigureAwait(false);
        if (startAuditFailure is not null)
        {
            var invalid = await TerminateAsync(
                ...
                actor,
                CustomLoopRunStatus.NeedsReview,
                startAuditFailure.FailureCode,
                startAuditFailure.Detail,
                terminalAuditRecorder: context.AuditRecorder).ConfigureAwait(false);
            return new RunAdvance(invalid.Run, invalid);
        }

        var waitEvidence = run.WaitEvidence.SingleOrDefault(item => item.ActivationOrdinal == activation.ActivationOrdinal);
        if (waitEvidence?.ContinuationEvidence is not null)
        {
            return await CompleteSequentialWaitNodeAsync(
                context,
                run,
                node,
                activation,
        ...
        if (_waitNodeExecutor is null)
        {
            var invalid = await TerminateAsync(
                run,
                actor,
                CustomLoopRunStatus.NeedsReview,
                "canonical_wait_executor_unavailable",
                "The canonical Wait executor is not composed; no sleeping checkpoint was published.",
                terminalAuditRecorder: context.AuditRecorder).ConfigureAwait(false);
            return new RunAdvance(invalid.Run, invalid);
        }

        GovernedLoopWaitParkResult parked;
        try
        {
            parked = await _waitNodeExecutor.ParkAsync(
                new GovernedLoopSequentialNodeDispatchRequest(
                    GovernedLoopSequentialNodeDispatchRequest.CurrentSchemaVersion,
                    context.Anchor,
                    context.Plan,
        ...
        {
            return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.Failed, run, $"Wait parking failed before a conclusive result: {SafeExceptionClass(exception)}."));
        }

        if (parked.Run is not { } durable)
        {
            return new RunAdvance(null, Result(
                parked.Status is GovernedLoopWaitParkResultStatus.Conflict or GovernedLoopWaitParkResultStatus.Invalid
                    ? CustomLoopOrderedRunStatus.InvalidState
                    : CustomLoopOrderedRunStatus.Failed,
                run,
                parked.Detail ?? $"Wait parking returned {parked.Status} without a canonical durable run."));
        }

        if (durable.Status == CustomLoopRunStatus.Cancelled)
        {
            return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.Cancelled, durable, parked.Detail ?? "The canonical Wait was cancelled before checkpoint publication."));
        }

        if (durable.Status == CustomLoopRunStatus.Failed)
        {
            return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.Failed, durable, parked.Detail ?? "The canonical Wait failed before checkpoint publication."));
        }
    ...
    private async Task<RunAdvance> CompleteSequentialWaitNodeAsync(
        ...
        string actor)
    {
        var retainedWait = run.WaitEvidence.SingleOrDefault(item => item.ActivationOrdinal == activation.ActivationOrdinal);
        if (retainedWait?.ContinuationEvidence is not { } continuation
            || retainedWait.ParkEvidence is null
            || !GovernedLoopWaitContractValidator.ValidateComposition(retainedWait.ParkEvidence, continuation).IsValid)
        {
            return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.InvalidState, run, "The Running Wait has no exact retained park and continuation evidence chain."));
        }

        var terminal = FindSequentialNodeEvidence(run, node, activation, attempt);
        var now = Now(run);
        var events = new List<CustomLoopRunEvent>();
        CustomLoopRunRecord outcomeOwner;
        if (terminal is null)
        {
            var sequentialNode = new SequentialNodeExecutionContext(
                context.Anchor.AdapterBinding,
            ...
            terminal = Event(
                ...
                now,
                CustomLoopRunEventKind.NodeAttemptCompleted,
                "The exact prepared wake continuation completed the canonical Wait activation.",
                activation.CycleIteration ?? run.Checkpoint.Iteration,
                node.NodeId,
                attempt) with
            {
            ...
            terminal = WithSequentialEvidence(
                terminal,
                sequentialNode,
                ...
                GovernedLoopControlCondition.Success);
            events.Add(terminal);
            outcomeOwner = run with { Events = [.. run.Events, terminal] };
        }
        else
        {
            if (terminal is not
                ...
                || !string.Equals(terminal.WaitContinuationEvidenceHash, continuation.ContentHash, StringComparison.Ordinal))
            {
                return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.InvalidState, run, "The retained Wait terminal event is absent or bound to different continuation evidence."));
            }

            outcomeOwner = run;
        }

        var completed = CompleteSequentialFrontier(outcomeOwner, context, node, attempt, attemptOperationId, now);
        if (completed is null)
        {
            return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.InvalidState, run, "The exact Wait completion could not advance its canonical frontier."));
        }

        events.AddRange(completed.SkipEvents);
        var waiting = completed.Frontier.Payload.Status == GovernedLoopFrontierStatus.Waiting;
        if (completed.Frontier.Payload.Status is not (GovernedLoopFrontierStatus.Active or GovernedLoopFrontierStatus.Waiting))
        {
            return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.InvalidState, run, "Wait completion produced an unsupported aggregate frontier posture."));
        }

        if (waiting && run.Status != CustomLoopRunStatus.Waiting)
        {
            var lifecycleOwner = run with { Events = [.. run.Events, .. events] };
            events.Add(Event(lifecycleOwner, now, CustomLoopRunEventKind.LifecycleChanged, "Ordered execution is parked because another canonical Wait remains pending."));
        }
        ...
        var persisted = await PersistAsync(run, candidate, IntegrityToken(), outcomeMayExist: false).ConfigureAwait(false);
        if (persisted.Terminal is not null)
        {
            CustomLoopRunRecord? latest;
            try
            {
                latest = await _runStore.GetAsync(run.Id, IntegrityToken()).ConfigureAwait(false);
            }
            catch
            {
                return persisted;
            ...
            if (latest?.Frontier?.Payload.Nodes.ElementAtOrDefault(activation.ActivationOrdinal) is not
                {
                    Status: GovernedLoopNodeExecutionStatus.Completed,
                    OutcomeEvidenceId: { } outcomeId,
                    OutcomeEvidenceHash: { } outcomeHash,
                }
                || !string.Equals(outcomeId, terminal.EventId, StringComparison.Ordinal)
                || !string.Equals(outcomeHash, terminal.SequentialNodeEvidence!.OutcomeArtifactHash, StringComparison.Ordinal))
            {
                return persisted;
            }

            persisted = new RunAdvance(latest, null);
        }

        var durable = persisted.Run!;
        var durableTerminal = FindSequentialNodeEvidence(durable, node, activation, attempt);
        if (durableTerminal is null
            || !string.Equals(durableTerminal.WaitContinuationEvidenceHash, continuation.ContentHash, StringComparison.Ordinal))
        {
            return new RunAdvance(null, Result(CustomLoopOrderedRunStatus.InvalidState, durable, "The Wait completion CAS returned without its exact canonical terminal evidence."));
        }

        var outcomeAuditFailure = await AppendOutcomeAuditAsync(
            durable,
            durableTerminal,
            CreateWaitNodeAudit(durable, actor, durableTerminal, AuditSchema.Outcomes.Succeeded),
            context.AuditRecorder,
            IntegrityToken()).ConfigureAwait(false);
        if (outcomeAuditFailure is not null)
        {
        ...
        return durable.Status == CustomLoopRunStatus.Waiting
            ? new RunAdvance(null, Result(CustomLoopOrderedRunStatus.Waiting, durable, "The resumed Wait completed once; another canonical Wait remains parked."))
            : new RunAdvance(durable, null);
    }
        ...
        string actor)
    {
        ...
        if (startAuditFailure is not null)
        {
            var invalid = await TerminateAsync(run, actor, CustomLoopRunStatus.NeedsReview, startAuditFailure.FailureCode, startAuditFailure.Detail, terminalAuditRecorder: context.AuditRecorder);
            return new RunAdvance(invalid.Run, invalid);
        }
        ...
        var outcomeAuditFailure = await AppendOutcomeAuditAsync(
            run,
            terminalEvidence,
            CreateTopologyNodeAudit(run, actor, terminalEvidence, AuditSchema.Outcomes.Succeeded),
            context.AuditRecorder,
            IntegrityToken());
        if (outcomeAuditFailure is not null)
        {
            var invalid = await TerminateAsync(run, actor, CustomLoopRunStatus.NeedsReview, outcomeAuditFailure.FailureCode, outcomeAuditFailure.Detail, terminalAuditRecorder: context.AuditRecorder);
            return new RunAdvance(invalid.Run, invalid);
        }
        ...
        DateTimeOffset? completedAtUtc = null)
    {
        ...
        var now = completedAtUtc ?? Now(run);
    ...
    private static string? CycleClaimDeadlineFailure(
        CustomLoopRunRecord run,
        ...
        DateTimeOffset now)
    {
        if (activation.CycleId is null || activation.CycleIteration is null)
        {
            return null;
        }

        var components = plan.Components
            .Where(component => string.Equals(component.CycleId, activation.CycleId, StringComparison.Ordinal))
            .Take(2)
            .ToArray();
        if (components.Length != 1 || components[0].MaximumDurationMilliseconds is not { } maximumDuration)
        {
            return "The exact admitted cycle duration budget could not be proven before another activation claim.";
        }

        var cycleStartedAtUtc = FindCycleStartedAtUtc(run, activation.CycleId);
        if (cycleStartedAtUtc is null
            && activation.CycleIteration == 1
            && node.ComponentTraversalOrdinal == 0)
        {
            // The first durable start event for the cycle entry establishes the elapsed-time clock.
            return null;
        }

        if (cycleStartedAtUtc is null
            || now < cycleStartedAtUtc.Value
            || (long)(now - cycleStartedAtUtc.Value).TotalMilliseconds > maximumDuration)
        {
            return "The exact admitted cycle duration budget was exhausted or could not be proven before another activation claim.";
        }

        return null;
    }

    private static int ProjectSequentialStepIndex(CustomLoopRunRecord run, CustomLoopInferenceStep step)
    {
        var stepIndex = Array.FindIndex(
            run.AdmittedDefinition.InferenceSteps,
            candidate => string.Equals(candidate.Id, step.Id, StringComparison.Ordinal));
        if (stepIndex < 0)
        {
            throw new InvalidOperationException("The canonical inference activation has no admitted legacy projection step.");
        }
        ...
        out CustomLoopRetainedOutput result)
    {
        ...
        try
        {
            content = JsonSerializer.Deserialize<string>(exact[0].Value.CanonicalValueJson);
        }
        catch (JsonException)
        {
            return false;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var iteration = completed.Iteration!.Value;
            var observed = run.Events.SingleOrDefault(item => exactStart is not null
                && item.Sequence > exactStart.Sequence
                ...
                && item.Attempt == attempt);
            var reservations = run.Events.Count(item => exactStart is not null
                && item.Sequence > exactStart.Sequence
                && item.Sequence < completed.Sequence
            ...
            try
            {
                ...
                assembly = _contextResolver.ResolveCanonicalInference(
                    run,
                    step,
                    activation,
                    exactBindings.Inputs,
            ...
            run = published.Run!;
            var nextStepIndex = ProjectSequentialStepIndex(run, step);
            var projectedRetained = retained with { Iteration = run.Checkpoint.Iteration };
            var earlier = run.Checkpoint.EarlierRetainedOutputs;
            var checkpoint = run.Checkpoint with
            {
                ...
                CurrentIterationResult = projectedRetained,
        ...
        if (exactTerminal?.SequentialNodeEvidence is { Kind: CustomLoopSequentialNodeEvidenceKind.AmbiguityAttention })
        {
            var ambiguous = await TerminateAsync(run, actor, CustomLoopRunStatus.NeedsReview, "canonical_outcome_reconciliation_failed", "The exact inference activation retained an ambiguous terminal outcome; automatic provider redispatch is forbidden.");
            return new RunAdvance(ambiguous.Run, ambiguous);
        }

        if (exactStart is not null)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (completed.ExitDecision != CustomLoopExitDecision.Complete)
        {
        ...
        return new RunAdvance(
            ...
            null,
            checkpoint,
            new PendingTerminal(CustomLoopRunStatus.Completed, null, detail, exactExitResult.Content));
    }
        ...
        SequentialNodeExecutionContext? sequentialNode = null)
    {
        ...
        var retainedIntegrityError = ValidateProviderResult(run, result, iteration, step.Id, attempt, out var durableToolRequestsConsumed, sequentialNode?.AttemptOperationId);
        ...
        run = published.Run!;
        var nextStepIndex = sequentialNode is null
            ? run.Checkpoint.NextStepIndex + 1
            : ProjectSequentialStepIndex(run, step);
        var projectedRetained = sequentialNode is null
            ? retained
            : retained with { Iteration = run.Checkpoint.Iteration };
        var earlier = sequentialNode is null
            && assembly.ResolvedOutputPolicy.RetainForLoopReasoning
            && !run.Checkpoint.EarlierRetainedOutputs.Any(item => item.Iteration == projectedRetained.Iteration
                && string.Equals(item.StepId, projectedRetained.StepId, StringComparison.Ordinal))
            ? [.. run.Checkpoint.EarlierRetainedOutputs, projectedRetained]
        ...
        SequentialNodeExecutionContext? sequentialNode = null)
    {
        ...
        if (iterationResult is null)
        {
            var terminal = await TerminateAsync(
                run,
                actor,
                sequentialNode is null ? CustomLoopRunStatus.Failed : CustomLoopRunStatus.NeedsReview,
                sequentialNode is null ? "missing_iteration_result" : "canonical_exit_binding_invalid",
                sequentialNode is null
                    ? "Deterministic Exit could not find the final inference result."
                    : "Canonical Exit could not resolve its exact activation-scoped Data input; checkpoint fallback is forbidden.");
            return new RunAdvance(terminal.Run, terminal);
        }
        ...
        bool reconcileRetainedDispatchStartAudit = true)
    {
        ...
        if (dispatchStartAlreadyRetained && reconcileRetainedDispatchStartAudit)
        {
            ...
            if (startAuditFailure is not null)
            {
                var review = await TerminateAsync(run, actor, CustomLoopRunStatus.NeedsReview, startAuditFailure.FailureCode, startAuditFailure.Detail);
                return new RunAdvance(review.Run, review);
            }
        }
        else if (!isExit && run.Checkpoint.NextStepIndex == 0)
        {
            events.Add(Event(run, now, CustomLoopRunEventKind.IterationStarted, $"Iteration {iteration} started in persisted step order.", iteration));
        }
        ...
        string actor)
    {
        ...
        var start = exactStart;
        var retainedRejection = FindSequentialNodeEvidence(run, node, activation, attempt);
        if (start is null
            || start.ProviderResponseId is null
            || FindSequentialDispatchStart(run, node, activation, attempt, start.EventId) is not { } authenticatedStart
            || !string.Equals(authenticatedStart.EventId, start.EventId, StringComparison.Ordinal)
            || retainedRejection is null
            || !string.Equals(retainedRejection.EventId, rejection.EventId, StringComparison.Ordinal)
            || rejection.Sequence <= start.Sequence)
        {
        ...
        try
        {
            ...
            else
            {
                ...
                var assembly = _contextResolver.ResolveCanonicalInference(
                    run,
                    step,
                    activation,
                    exactBindings.Inputs,
    ...
    private async Task<CustomLoopOrderedRunResult> AppendTerminalIntegrityWarningAsync(
        CustomLoopRunRecord terminalRun,
        CustomLoopOrderedRunStatus resultStatus,
        string warningDetail)
    {
        ...
        try
        {
            var persisted = await _runStore.AppendTerminalIntegrityWarningAsync(
                terminalRun.Id,
                terminalRun.LifecycleVersion,
                warning,
                IntegrityToken());
            ...
            {
                return Result(resultStatus, persisted.Run, warningDetail);
            }

            if (persisted.Status is CustomLoopRunStoreStatus.Conflict or CustomLoopRunStoreStatus.TerminalImmutable)
            {
                var latest = await _runStore.GetAsync(terminalRun.Id, IntegrityToken());
                var withoutWarning = latest is null ? null : WithoutTerminalIntegrityWarning(latest);
                if (latest?.Events.LastOrDefault() is { Kind: CustomLoopRunEventKind.IntegrityWarning } durableWarning
                    && withoutWarning is not null
                    && CustomLoopRunValidator.HasSameDurableVersion(terminalRun, withoutWarning))
                {
                    return Result(resultStatus, latest, durableWarning.Detail);
                }
            }

            return Result(resultStatus, terminalRun, $"{warningDetail} The integrity warning could not be durably appended ({persisted.Status}).");
        }
        catch (Exception exception)
        {
            return Result(resultStatus, terminalRun, $"{warningDetail} The integrity-warning persistence outcome is uncertain: {SafeExceptionClass(exception)}.");
        }
    }
        ...
        CustomLoopRunEvent terminalEvent)
    {
        ...
        if (current is null)
        {
            ...
            {
                var aggregate = GovernedLoopSequentialFrontierMachine.ReviewBlockAggregate(
                    frontier,
                    binding,
                    terminalEvent.TimestampUtc);
                return aggregate.Status == GovernedLoopSequentialFrontierTransitionStatus.Applied
                    ? aggregate.Frontier
                    : null;
            }
        ...
        var exactOutcome = FindExactClosedSequentialOutcome(run, current);
        var exactEvidence = exactOutcome?.SequentialNodeEvidence;
    ...
    private static AuditEvent CreateWorkspaceActionAudit(
        CustomLoopRunRecord run,
        string actor,
        CustomLoopRunEvent runEvent,
        string outcome)
    {
        var evidence = runEvent.SequentialNodeEvidence
            ?? throw new InvalidOperationException("Workspace Action audit requires exact sequential evidence.");
        var metadata = RunMetadata(run);
        ...
        metadata["sequentialEvidenceHash"] = evidence.EvidenceHash;
        metadata["modelDispatched"] = false;
        metadata["workspaceActionResultHash"] = runEvent.CanonicalOutput is null
            ? null
            : CustomLoopTraceContentHash.Compute(runEvent.CanonicalOutput);
        return new AuditEvent(
            ...
            run.Id,
            outcome,
            "Canonical workspace Action evidence reached a bounded durable disposition.",
            metadata);
    }

    private static AuditEvent CreateCommandActionAudit(
        CustomLoopRunRecord run,
        string actor,
        CustomLoopRunEvent runEvent,
        string outcome)
    {
        var evidence = runEvent.SequentialNodeEvidence
            ?? throw new InvalidOperationException("Command Action audit requires exact sequential evidence.");
        var metadata = RunMetadata(run);
        metadata["iteration"] = runEvent.Iteration;
        metadata["stepId"] = runEvent.StepId;
        metadata["attempt"] = runEvent.Attempt;
        ...
        metadata["sequentialEvidenceHash"] = evidence.EvidenceHash;
        metadata["modelDispatched"] = false;
        metadata["commandActionResultHash"] = runEvent.CanonicalOutput is null
            ? null
            : CustomLoopTraceContentHash.Compute(runEvent.CanonicalOutput);
        return new AuditEvent(
            runEvent.TimestampUtc.ToUniversalTime(),
            ...
            run.Id,
            outcome,
            "Canonical structured command Action evidence reached a bounded durable disposition.",
            metadata);
    }
    ...
    private static AuditEvent CreateTopologyNodeAudit(
        CustomLoopRunRecord run,
        string actor,
        CustomLoopRunEvent runEvent,
        string outcome)
    {
        var evidence = runEvent.SequentialNodeEvidence
            ?? throw new InvalidOperationException("Topology-node audit requires exact sequential evidence.");
        var metadata = RunMetadata(run);
        metadata["iteration"] = runEvent.Iteration;
        metadata["stepId"] = runEvent.StepId;
        metadata["attempt"] = runEvent.Attempt;
        ...
        metadata["sequentialEvidenceHash"] = evidence.EvidenceHash;
        metadata["modelDispatched"] = false;
        metadata["controlOutcome"] = evidence.ControlOutcome?.ToString().ToLowerInvariant();
        return new AuditEvent(
            runEvent.TimestampUtc.ToUniversalTime(),
            actor,
            AuditSchema.Actions.LoopNodeAttempt,
            run.Id,
            outcome,
            "Deterministic topology-node evidence is durable.",
            metadata);
    }

    private static AuditEvent CreateWaitNodeAudit(
        CustomLoopRunRecord run,
        string actor,
        CustomLoopRunEvent runEvent,
        string outcome)
    {
        var evidence = runEvent.SequentialNodeEvidence
            ?? throw new InvalidOperationException("Wait-node audit requires exact sequential evidence.");
        var metadata = RunMetadata(run);
        metadata["iteration"] = runEvent.Iteration;
        metadata["stepId"] = runEvent.StepId;
        metadata["attempt"] = runEvent.Attempt;
        ...
        metadata["sequentialEvidenceHash"] = evidence.EvidenceHash;
        metadata["modelDispatched"] = false;
        metadata["continuationEvidenceHash"] = runEvent.WaitContinuationEvidenceHash;
        return new AuditEvent(
            runEvent.TimestampUtc.ToUniversalTime(),
            actor,
            AuditSchema.Actions.LoopNodeAttempt,
            ...
            outcome == AuditSchema.Outcomes.Started
                ? "Canonical Wait dispatch evidence is durable before sleeping ownership is released."
                : "Canonical Wait continuation evidence is durable and completed exactly once.",
            metadata);
    }
        ...
        IGovernedLoopSequentialAuditRecorder auditRecorder)
    {
        ...
        return start is null
            ...
            : AppendDeterministicNodeStartAuditAsync(
                run,
                start,
    ...
    private static Task<SequentialAuditBoundaryFailure?> ReconcileTopologyNodeStartAuditAsync(
        CustomLoopRunRecord run,
        GovernedLoopSequentialPlanNode node,
        GovernedLoopNodeExecutionEvidence activation,
        int attempt,
        ...
        IGovernedLoopSequentialAuditRecorder auditRecorder)
    {
        ...
        return start is null
            ? Task.FromResult<SequentialAuditBoundaryFailure?>(new SequentialAuditBoundaryFailure(
                "canonical_start_audit_conflict",
                "The deterministic topology-node attempt has no exact authenticated start marker for append-once audit reconciliation."))
            : AppendDeterministicNodeStartAuditAsync(
                run,
                start,
                CreateTopologyNodeAudit(run, actor, start, AuditSchema.Outcomes.Started),
                auditRecorder,
                IntegrityToken());
    }

    private static Task<SequentialAuditBoundaryFailure?> ReconcileWaitNodeStartAuditAsync(
        CustomLoopRunRecord run,
        GovernedLoopSequentialPlanNode node,
        GovernedLoopNodeExecutionEvidence activation,
        int attempt,
        ...
        IGovernedLoopSequentialAuditRecorder auditRecorder)
    {
        ...
        return start is null
            ? Task.FromResult<SequentialAuditBoundaryFailure?>(new SequentialAuditBoundaryFailure(
                "canonical_start_audit_conflict",
                "The Wait attempt has no exact authenticated start marker for append-once audit reconciliation."))
            : AppendDeterministicNodeStartAuditAsync(
                run,
                start,
                CreateWaitNodeAudit(run, actor, start, AuditSchema.Outcomes.Started),
                auditRecorder,
                IntegrityToken());
    }

    private static async Task<SequentialAuditBoundaryFailure?> AppendDeterministicNodeStartAuditAsync(
        CustomLoopRunRecord run,
        ...
        CancellationToken cancellationToken)
    {
            ...
            || !CustomLoopSequentialOutcomeArtifactHash.Matches(startEvent))
        {
            return new SequentialAuditBoundaryFailure(
                "canonical_start_audit_conflict",
                "The deterministic node start could not identify one exact append-once audit operation.");
        }
        ...
        catch (Exception exception)
        {
            return new SequentialAuditBoundaryFailure(
                ...
                $"The append-once deterministic node start audit could not prove durability: {SafeExceptionClass(exception)}.");
        }
        ...
        return recorded?.Status switch
        {
            ...
            GovernedLoopSequentialAuditRecordStatus.Conflict => new SequentialAuditBoundaryFailure(
                "canonical_start_audit_conflict",
                "The append-once deterministic node start audit operation is already bound to divergent evidence."),
            _ => new SequentialAuditBoundaryFailure(
                "canonical_start_audit_unavailable",
                "The append-once deterministic node start audit could not prove a durable result."),
        };
    }
    ...
    private static AuditEvent AttemptAudit(
        ...
        CustomLoopExitDecision? exitDecision = null,
        int attempt = 1,
        CustomLoopRunEvent? exactStart = null,
        CustomLoopRunEvent? exactTerminal = null)
    {
        ...
        var authority = exactStart?.ToolAuthority
            ?? run.Events.LastOrDefault(item => item.Kind is CustomLoopRunEventKind.NodeAttemptStarted or CustomLoopRunEventKind.ExitDecisionStarted
                && item.Iteration == iteration
                && string.Equals(item.StepId, stepId, StringComparison.Ordinal)
        ...
        var sequentialEvidence = exactTerminal?.SequentialNodeEvidence
            ?? run.Events.LastOrDefault(item => item.Iteration == iteration
                && string.Equals(item.StepId, stepId, StringComparison.Ordinal)
                && item.Attempt == attempt
    ...
    private static string? ValidateProviderResult(
        CustomLoopRunRecord run,
        CustomLoopInferenceAttemptResult result,
        int iteration,
        string stepId,
        int attempt,
        out int durableToolRequestsConsumed,
        string? exactAttemptOperationId = null)
    {
        ...
        var attemptStart = run.Events.LastOrDefault(item => (item.Kind is CustomLoopRunEventKind.NodeAttemptStarted or CustomLoopRunEventKind.ExitDecisionStarted)
            ...
            && string.Equals(item.StepId, stepId, StringComparison.Ordinal)
            && item.Attempt == attempt
            && (exactAttemptOperationId is null || string.Equals(item.EventId, exactAttemptOperationId, StringComparison.Ordinal)));
        ...
        IGovernedLoopSequentialAuditRecorder auditRecorder)
    {
        ...
        try
        {
            ...
            var commandActionCapabilityIds = artifact.Graph.Nodes
            ...
            if (!Equals(binding.ExecutionBinding.Revision, plan.Revision)
                ...
                || !binding.CommandActionCapabilityIds.SequenceEqual(commandActionCapabilityIds, StringComparer.Ordinal))
            {
                return null;
            }
    ...
    private static bool SequentialRunMatches(CustomLoopRunRecord run, SequentialExecutionContext context)
    {
        ...
        if (!string.Equals(run.Id, binding.ExecutionBinding.RunId, StringComparison.Ordinal)
            ...
            || !IsExactSequentialCapabilitySet(
                context.AllowedCapabilityIds,
                binding.AdmissionReceipt,
                binding.CommandActionCapabilityIds,
                invocation.TriggerOrigin is not null,
                context.Plan.Nodes.Any(item => Equals(item.Descriptor, GovernedLoopSequentialNodeDescriptors.ProviderInference)))
        ...
        bool hasInference)
    {
        ...
        var profileIds = admissionReceipt.Evidence.ModelRoutingAdmission.Entries
            ...
            .Distinct(StringComparer.Ordinal)
            .ToArray();
        var toolFree = new[] { SequentialConversationTurnCapabilityId }
            .Concat(hasInference ? [SequentialModelInferenceCapabilityId] : [])
            .Concat(scheduled ? [SequentialScheduleTriggerCapabilityId] : [])
            .Concat(hasInference ? profileIds : [])
            .Concat(commandActionCapabilityIds)
            .Distinct(StringComparer.Ordinal)
            .Order(StringComparer.Ordinal)
            .ToArray();
