...
public sealed class WorkspaceActionNativeHost : IWorkspaceActionNativeHost
{
    ...
    public async Task<int> CleanupOrphansAsync(int maximumArtifacts, CancellationToken cancellationToken = default)
    {
        ...
        for (var index = 0; index < examinationCount; index++)
        {
            ...
            if (!targetUnchanged)
            {
                try
                {
                    using var session = WorkspaceActionRetainedTargetSession.Open(_rootPath, _scopeId, target!, writeTarget: false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var current = WorkspaceActionRetainedTargetSession.Open(_rootPath, _scopeId, target!, writeTarget: false);
                ...
                token => dispatchBoundary.CrossAsync(
                    async boundaryToken =>
                    {
                        ...
                        else if (windowsAtomicReplacement)
                        {
                            ...
                            using var displaced = WorkspaceActionNativeFileSystem.OpenRelativeFile(stage.Directory, displacedName, allowMissing: false, write: true)!;
                        ...
                        if (before.EntryKind == WorkspaceActionEntryKind.RegularFile)
                        {
                            WorkspaceActionNativeFileSystem.DeleteExact(
                                stage.Directory,
                                windowsAtomicReplacement ? stage.Name + ".displaced" : stage.Name,
                                session.TargetHandle!,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (entries.Any(name =>
            ...
            || displacedReferences.Overlaps(stagedReferences))
        {
        ...
        try
        {
            ...
            file = WorkspaceActionNativeFileSystem.OpenRelativeFile(directory, stageName, allowMissing: true, write: true);
            if (file is null)
            {
                file = WorkspaceActionNativeFileSystem.CreateRelativeFile(directory, stageName);
                createdFile = true;
