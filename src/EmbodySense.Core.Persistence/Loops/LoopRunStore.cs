...
public sealed class LoopRunStore : ILoopRunStore
{
    ...
    private static async Task<LoopRunRecord> ReadRunAsync(string path, CancellationToken cancellationToken)
    {
        ...
        var run = await JsonSerializer.DeserializeAsync<LoopRunRecord>(stream, JsonOptions, cancellationToken)
    ...
    private static void ValidateRun(LoopRunRecord run)
    {
        ...
        ArgumentException.ThrowIfNullOrWhiteSpace(run.Status);
        ArgumentException.ThrowIfNullOrWhiteSpace(run.Surface);
        ArgumentException.ThrowIfNullOrWhiteSpace(run.Trigger);
